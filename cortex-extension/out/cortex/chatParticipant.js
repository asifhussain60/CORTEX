...
exports.CortexChatParticipant = void 0;
const vscode = __importStar(require("vscode"));
class CortexChatParticipant {
    constructor(brainBridge, conversationCapture, isOnline = false) {
    ...
    async handleCommand(command, request, stream) {
        switch (command) {
            case 'resume':
    ...
    async checkpointCommand(stream) {
        if (!this.isOnline) {
    ...
    async platformSwitchCommand(platform, stream) {
        if (!this.isOnline) {
        ...
        try {
            ...
            const response = await this.generateResponse(naturalLanguage, {});
            stream.markdown(response.content);
    ...
    async resumeLastConversation() {
        ...
        vscode.window.showInformationMessage(`Ready to resume: ${lastConversation.topic}. Type @cortex /resume to continue.`);
    }
    ...
    async offerResume() {
        ...
        if (hoursSince < 24) {
            const response = await vscode.window.showInformationMessage(`Resume last conversation: "${lastConversation.topic}"? (${Math.floor(hoursSince)}h ago)`, 'Resume', 'Dismiss');
            if (response === 'Resume') {
...
exports.CortexChatParticipant = CortexChatParticipant;

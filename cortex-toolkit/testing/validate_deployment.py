...
class DeploymentValidator:
    ...
    def check_response_template_wiring(self):
        ...
        if not templates_file.exists():
            ...
            try:
                ...
                elif 'routing:' not in templates_content and 'sections:' not in templates_content:
                    wiring_issues.append("response-templates-v4.yaml missing 'routing:' or 'sections:' section")
                
                # Check for key sections in v4 format
                required_sections = ['routing', 'sections', 'components', 'adaptive_structure']
                missing_sections = [t for t in required_sections if t not in templates_content]
                if missing_sections:
                    wiring_issues.append(f"Missing critical sections: {', '.join(missing_sections)}")
        ...
        if not cortex_prompt.exists():
            ...
            try:
                ...
                # Check for v4 template reference (or legacy for backwards compat)
                if 'response-templates' not in prompt_content:
                    wiring_issues.append("CORTEX.prompt.md does not reference response-templates")
                
                if 'RESPONSE' not in prompt_content.upper():
                    wiring_issues.append("CORTEX.prompt.md missing response format guidance")
    ...
    def check_tdd_mastery_components(self):
        ...
        if not templates.exists():
            ...
            try:
                ...
                # Check for TDD-related sections (v4 format)
                tdd_sections = ['tdd', 'test', 'validation']
                missing_sections = [t for t in tdd_sections if t not in templates_content.lower()]
                
                if missing_sections:
                    tdd_issues.append(f"response-templates-v4.yaml missing TDD sections: {', '.join(missing_sections)}")
            except Exception as e:
    ...
    def check_tdd_mastery_enhancements(self):
        ...
        # Check response templates have TDD workflow support (v4 format)
        templates = self.project_root / "cortex-brain" / "response-templates-v4.yaml"
        if templates.exists():
            try:
                with open(templates, 'r', encoding='utf-8') as f:
                ...
                # Check for TDD-related sections in v4 format
                tdd_sections = [
                    'tdd',  # TDD section
                ...
                if missing_sections:
                    enhancement_issues.append(f"response-templates-v4.yaml missing TDD support: {', '.join(missing_sections)}")
            except Exception as e:

...
#### 11.6 Comprehensive Validation Script (MANDATORY)

**⚠️ CRITICAL: Run this script after EVERY decomposition to verify plan integrity!**

This PowerShell script validates the overall decomposition plan integrity, ensuring all content is preserved, entry points work, and structure is correct.

**Script Location:** `cortex-toolkit/scripts/validate_decomposition.ps1`
...
2. **Exit codes:**
   ...
   - `1` = Validation failed (DO NOT proceed, rollback recommended)
...
3. **Automated integration:**
   ...
   - name: Validate Decomposition
     run: |
       $result = .\cortex-toolkit\scripts\validate_decomposition.ps1 -OriginalFile $orig -IndexFile $index -DecompFolder $decomp -ArtifactType $type
       if ($LASTEXITCODE -ne 0) { throw "Decomposition validation failed" }
   ```

**What the script validates:**

✅ **Entry Point Preservation** - Original file path unchanged  
✅ **Content Preservation** - Line count, headings, code blocks, tables  
✅ **File Structure** - Expected subfolders exist  
✅ **Index Completeness** - Required sections present  
✅ **Reference Integrity** - All referenced files exist  
✅ **Archive Creation** - Backup exists for rollback  
✅ **Markdown Syntax** - No unclosed blocks, broken links  

...
**Version:** 16.0 | **Updated:** 2026-02-14 | **Mode:** HEXA-MODE (PRE-FLIGHT + AUDIT + META-AUDIT + DIGEST + QUERY + PLAN + DESIGN) | **Status:** ACTIVE | **Silent Autonomous:** ✅ | **Visual Progress:** ASCII Bars | **Incremental TDD:** ✅ | **Token Optimization:** ✅ | **Holistic Validation:** Phase 48 ✅
...
#### VS Code MCP Configuration (Auto-Generated by setup-mcp.py)

**CRITICAL: `.vscode/settings.json` is NOT committed to git (cross-platform policy)**

Each machine generates its own platform-specific configuration via `python .cortex/setup-mcp.py`:
...
- Committing wrong path → MCP breaks on other platform

**Auto-regeneration:**
- `.githooks/post-checkout` runs `setup-mcp.py` on every `git pull`
- Ensures correct paths after switching branches or pulling changes

---

## 🔴 MANDATORY CROSS-PLATFORM AUDIT (CORE-051)

**Authority:** CORE-051 | **Enforcement:** BLOCKING | **Added:** 2026-02-12

### Audit Check (EXECUTE ON EVERY /audit COMMAND)

**Step 1: Verify `.vscode/settings.json` NOT in git**
```bash
git ls-files | grep ".vscode/settings.json"
# Expected: (empty) - file should NOT be tracked
```
**If tracked:** Run `git rm --cached .vscode/settings.json && git commit -m "Fix: Remove platform-specific settings from git"`

**Step 2: Verify cross-platform paths in setup-mcp.py**
```python
# Windows detection (line ~503)
if IS_WINDOWS:
...
**Step 3: Verify git hooks configured**
```bash
git config core.hooksPath
# Expected: .githooks
```

**Step 4: Verify post-checkout calls setup-mcp.py**
```bash
grep "setup-mcp.py" .githooks/post-checkout
# Expected: python .cortex/setup-mcp.py
```
...
**Action:** Run `python .cortex/setup-mcp.py` (cross-platform)  

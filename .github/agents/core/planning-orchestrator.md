...
def execute_wave_tracks(phase: Phase) -> WaveExecutionResult:
    """
    Execute all stages in phase. Respects max_parallel_stages limit.
    Returns aggregated result.
    """
    # Step 1: Wait for phase to be ready
    if not evaluate_phase_start_condition(phase_obj):
        mark_phase_blocked(phase_obj)
...
def sync_phase_completion(phase: Phase, execution_result: ExecutionResult):
    ...
    for dependent_phase in find_dependent_phases(phase.parent_phase):
        if evaluate_phase_start_condition(dependent_phase):
            notify_phase_ready(dependent_phase)
```

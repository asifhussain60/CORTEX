...
def audit_plan_reality_delta():
    ...
    for phase in plan.phases:
        for dependency in phase.dependencies:
            if dependency.status == "INCOMPLETE" and phase.status == "WAITING":
                report.add_drift(
                    type="BLOCKED_DEPENDENCY",
                    phase_id=phase.id,
                    blocker=dependency.id,
                    recommendation="Unblock or reorder phase"
                )
...
def reorganize_waves():
    ...
    # Step 4: Cluster phases into groups using bin-packing + ROI
    phase_groups = []
...
async def execute_wave_autonomous(phase_id: str):
    """
    Execute entire phase group autonomously, managing state and continuation.
    
    Flow:
    1. Load phase group definition + dependencies
    2. For each phase in phase_group:
    ...
    """
    phase_group = load_phase_group(phase_id)
    ...
    for phase in phase_group.phases:
        # Pre-phase check: Token budget
        if tokens_used > token_limit:
            report.status = "CHECKPOINT_REACHED"
            report.continuation_point = {
                "phase_id": phase_id,
                "next_phase_index": phase_group.phases.index(phase),
                "tokens_used": tokens_used,
                "phases_completed": [p.id for p in phase_group.phases[:phase_group.phases.index(phase)]],
                "command": f"@cortex /plan continue phase-{phase_id}"
            }
            return report
    ...
    report.metrics = {
        "phases_executed": len(phase_group.phases),
        "total_tokens": tokens_used,
        "estimated_vs_actual_accuracy": compute_accuracy(phase_group, report),
...
```
User: /plan execute phase-3
      ↓
Phase Resolver identifies:
  - What: Execute phase-3 (phases 45-47)
...
```python
"""Execute phase phases autonomously with state management."""

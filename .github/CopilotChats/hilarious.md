# 🧠 Prompt: Rewrite Technical Narrative into a Hilarious Story Format

You are an AI assistant helping me turn a technical narrative into a *comedic*, *story-driven*, and *entertaining* explanation of a project called **CORTEX**.

### 🎯 Goal:
Transform a serious, explanatory story about CORTEX — a memory system designed to enhance GitHub Copilot — into a *funny, sitcom-style narrative* that showcases its capabilities through dialogue and absurd situations.

### 🤖 Setting:
- The main character is **Asif Codenstein**, a brilliant but chaotic software developer working in his New Jersey basement.
- He is trying to give GitHub Copilot a brain — a persistent memory system called **CORTEX**.
- His wife, calling in from the UK, serves as the sarcastic voice of reason.
- Copilot is personified as a **snarky, forgetful, overly-helpful robot intern** with amnesia.
- The story should blend real technical problems with ridiculous storytelling (e.g., coffee mugs as memory tiers, sentient code, etc.)

### ✍️ Instructions:
Rewrite the original story into a **funny, punchy narrative** that:
- Keeps the core technical ideas intact
- Introduces CORTEX’s features *through character dialogue and problems*
- Uses clever analogies and metaphors (e.g., Copilot as a goldfish, memory layers as coffee mugs)
- Embodies a tone like: *Silicon Valley* × *The Office* × *Black Mirror (but funny)*

### 🧩 Example Tone:
> "Copilot, what did we do yesterday?"  
> *"I have no memory of this. Want to build a new auth system?"*  
> "WE BUILT ONE YESTERDAY."  
> *"Neat. Let’s do it again, but worse."*

### 💾 Output Format:
- Title the chapter or section (e.g., "Prologue: Your Robot Has the Memory of a Goldfish")
- Use Markdown headers for structure (e.g., `##`, `###`)
- Include dialogue in blockquote format with speaker name (e.g., `> COPILOT:`)
- Use inline code blocks for terminal/code snippets (e.g., `git commit -m`)
- List features in bullet points when revealed (e.g., what CORTEX adds)

### 🧪 Feature Reveal Strategy:
Each scene or mishap should introduce a key CORTEX capability:
- Tiered memory (T1: active, T2: scoped, T3: long-term)
- Knowledge graph recall
- Safety layer (SKULL protocol)
- Modular memory lanes per project
- Emotional damage control (optional)

---

### 📝 Input:
Use the following original content as the source to be transformed:

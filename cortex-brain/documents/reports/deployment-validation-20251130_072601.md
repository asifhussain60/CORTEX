...
**Message:** Found 1 mock/stub patterns in production code paths. These must have real implementations, not just be removed. Deployment BLOCKED until proper functionality exists.
...
```json
{
  ...
  "issues": [
    "Could not validate brain protection rules: 'charmap' codec can't decode byte 0x9d in position 6915: character maps to <undefined>"
  ],
  "passed_checks": 5,
  "total_checks": 6
}
...
```json
{
  ...
  "issues": [
    "tdd-mastery-guide.md doesn't document git checkpoint integration"
  ]
}
```
...
- Found 1 mock/stub patterns in production code paths. These must have real implementations, not just be removed. Deployment BLOCKED until proper functionality exists.

...
def main():
    ...
    if not schema_path.exists():
        ...
        # Use config if available, fallback to relative path
        try:
            import sys
            from pathlib import Path as P
            sys.path.insert(0, str(P(__file__).parent.parent.parent.parent.parent / 'src'))
            from config.dashboard_config import get_config
            config = get_config()
            schema_dir = config.get_path('schema')
            schema_path = schema_dir / "health-data-schema.json"
        except:
            schema_path = Path("cortex-brain/dashboards/data/schema/health-data-schema.json")

...
**Compliance Scope:** Healthcare reimbursement accounts (FSA/HSA/HRA) require validation against:
- **IRS Tax Code:** Contribution limits, carryover rules, qualified expenses
- **HIPAA Security Rule:** PHI data protection, access controls, audit trails
- **PCI-DSS v4.0.1:** Payment card data security (if card transactions supported)

**P0 Gap Validated:** No code found validating IRS contribution limits ($3,200 FSA, $4,150/$8,300 HSA) - production risk confirmed.

---

## 1. IRS Regulatory Requirements

### 1.1 Contribution Limits (IRC §125, IRC §223, IRS Pub 969)

| Regulation | 2024 Limit | 2025 Limit | Business Rule | Code Must Validate | Test Scenario | Priority |
|------------|------------|------------|---------------|-------------------|---------------|----------|
| **FSA Annual Contribution** | $3,200 | TBD (Nov 2025) | Max employee + employer contributions | YES | Reject contribution >$3,200 | **P0** |
| **FSA Carryover** | $640 | TBD | Unused funds exceeding $640 forfeited | YES | Test year-end forfeiture logic | **P0** |
| **HSA Self-Only** | $4,150 | $4,300 | Individual coverage limit | YES | Reject contribution >$4,150 (2024) | **P0** |
| **HSA Family** | $8,300 | $8,550 | Family coverage limit | YES | Reject contribution >$8,300 (2024) | **P0** |
| **HSA Catch-Up (55+)** | $1,000 | $1,000 | Additional for age 55+ | YES | Allow +$1,000 if age ≥55 | **P1** |
...
| **Dependent Care FSA** | $5,000 | $5,000 | Per household (not indexed) | YES | Reject >$5,000 household | **P0** |

**Source:** IRS Publication 969 (2024), Revenue Procedure 2023-34  
...
### 1.2 Carryover & Grace Period Rules (26 CFR §125-5, IRS Pub 969)

| Regulation | Rule | Business Logic | Code Must Implement | Test Scenario | Priority |
|------------|------|----------------|---------------------|---------------|----------|
| **FSA Carryover vs Grace** | Employer chooses ONE (not both) | Mutual exclusion | YES | Prevent both enabled | **P0** |
| **FSA Carryover Amount** | Maximum $640 (2024) | Unused funds >$640 forfeited | YES | Test year-end with $640, $641 | **P0** |
| **FSA Grace Period** | Maximum 2.5 months | Jan 1 - Mar 15 grace | YES | Allow claims through Mar 15 | **P1** |
| **FSA Use-It-Or-Lose-It** | Unused funds forfeited | Transfer to employer | YES | Test forfeiture to employer account | **P2** |
| **HSA Rollover** | 100% unlimited | Never forfeited | YES | Verify 100% carryover year-to-year | **P1** |
| **HRA Carryover** | Employer discretion | Plan-specific rules | YES | Validate per-plan carryover config | **P2** |
| **Dependent Care FSA Carryover** | NO CARRYOVER | Strict use-it-or-lose-it | YES | Test $0 carryover (no $640 option) | **P0** |

**Source:** IRS Publication 969, IRS Notice 2013-71 (carryover option)  
**Status:** ⏳ **PARTIAL** - Carryover logic found in `CarryoverDollarsDomainService.cs` (20 methods), but grace period logic not yet validated  
**Next Step:** Scan year-end processing in Batch 7-8 to validate grace period implementation

### 1.3 Qualified Medical Expenses (IRS Pub 502, IRC §213)
...
**Source:** IRS Publication 502 (2024)  
**Status:** ⏳ **UNKNOWN** - Claims validation logic not yet scanned  
...
|-------------|------|----------|----------------------|---------------|----------|
| **HSA Contributions** | Form 8889 | Tax return filing | Report to IRS | Verify W-2 Box 12 Code W | **P0** |
| **HSA Distributions** | Form 1099-SA | Jan 31 | Trustee reports | Verify distribution reporting | **P0** |
| **Excess Contributions** | Form 5329 (6% excise tax) | Tax return filing | Notify participant | Test excess contribution alert | **P1** |
| **Archer MSA** | Form 8853 | Tax return filing | Report to IRS | Verify MSA reporting | **P2** |

**Status:** ⏳ **UNKNOWN** - Tax reporting not yet scanned  
**Next Step:** Batch 11-12 - Scan reporting services for IRS form generation
...
**Source:** HIPAA Security Rule, 45 CFR Part 164 Subpart C  
...
| **Person or Entity Authentication** | Verify person/entity is who they claim to be | MFA, strong authentication | Test MFA requirement for PHI access | **P0** |
...
**Status:** ⏳ **UNKNOWN** - Security implementation not yet scanned  
**Next Step:** Batch 13-14 - Scan security services for HIPAA compliance

**HIPAA PHI Examples in RA Domain:**
- Account balances (FSA/HSA/HRA balances are PHI)
- Claims data (reimbursement requests contain medical procedure codes)
- Transaction history (dates/amounts linked to medical services)
- Participant demographic data (SSN, DOB, address)

---

## 3. PCI-DSS v4.0.1 Requirements (Payment Card Data)
...
**Scope Decision Needed:** Does HealthEquity RA system support card transactions (debit cards for FSA/HSA)?  
...
|--------|-------------|------------|--------------------------|-------------|-------|
| **P0-001** | No FSA $3,200 limit validation | IRS Pub 969, IRC §125 | Contribution processing service | Create test: Reject $3,201 contribution | Batch 3-6 |
| **P0-002** | No HSA $4,150/$8,300 limit validation | IRS Pub 969, IRC §223 | Contribution processing service | Create test: Reject $4,151 self-only | Batch 3-6 |
| **P0-003** | No FSA $640 carryover limit | IRS Pub 969, 26 CFR §125-5 | Year-end processing service | Create test: Forfeit $641+ unused funds | Batch 7-8 |
| **P0-004** | Dependent Care FSA no carryover validation | IRS Pub 969 | Dependent Care FSA service | Create test: Verify $0 carryover (not $640) | Batch 7-8 |
| **P0-005** | Cosmetic surgery rejection | IRS Pub 502 | Claims validation service | Create test: Reject liposuction claim | Batch 9-10 |
| **P0-006** | CVV storage prohibition | PCI-DSS Req 3 | Card transaction service | Create test: Verify CVV never stored | Batch 15-16 |
| **P0-007** | PAN encryption at rest | PCI-DSS Req 3 | Card storage service | Create test: Verify encrypted PAN in DB | Batch 15-16 |
| **P0-008** | PHI audit trail | HIPAA §164.312 | Security/logging service | Create test: Verify audit log for balance view | Batch 13-14 |
| **P0-009** | MFA for PHI access | HIPAA §164.312 | Authentication service | Create test: Require MFA for account access | Batch 13-14 |

### 4.2 High-Priority Validation (P1)

| Gap ID | Description | Regulation | Code Location (Expected) | Test Design | Batch |
|--------|-------------|------------|--------------------------|-------------|-------|
| **P1-001** | HSA catch-up ($1,000 age 55+) | IRS Pub 969 | Contribution service | Test: Allow $5,150 if age=55 | Batch 3-6 |
| **P1-002** | HDHP min deductible validation | IRS Pub 969 | HSA eligibility service | Test: Reject if deductible <$1,600 | Batch 3-6 |
| **P1-003** | FSA grace period (2.5 months) | 26 CFR §125-5 | Year-end processing | Test: Allow claims through Mar 15 | Batch 7-8 |
| **P1-004** | OTC medicine qualification (2019+) | IRS Pub 502 | Claims validation | Test: Approve aspirin claim (post-2019) | Batch 9-10 |
| **P1-005** | Session timeout for PHI | HIPAA §164.312 | Session management | Test: Timeout after 15 min inactivity | Batch 13-14 |
| **P1-006** | PAN masking in UI | PCI-DSS Req 3 | Card display service | Test: Show XXXX-XXXX-XXXX-1234 only | Batch 15-16 |
...
**Regulatory Focus:**
- IRS Pub 969 limits ($3,200 FSA, $4,150/$8,300 HSA, $1,000 catch-up)
...
**Regulatory Focus:**
- 26 CFR §125-5 ($640 carryover OR 2.5-month grace, not both)
- Use-it-or-lose-it enforcement

**Expected Findings:**
- ✅ Carryover logic exists (`CarryoverDollarsDomainService.cs`) - VALIDATE against $640 limit
- ⏳ Grace period logic - LOCATE and VALIDATE 2.5-month window

### Batch 9-10: Claims Processing
**Scan:**
- Claims validation services
- Qualified expense determination
- 2019+ OTC medicine rules

**Regulatory Focus:**
- IRS Pub 502 (qualified expenses, OTC rule changes, preventive care)
- Notice 2024-75 (OTC contraceptives, breast cancer screening, CGMs)

**Expected Findings:**
- ⏳ Qualified expense list - VALIDATE against IRS Pub 502
- ⚠️ Cosmetic surgery rejection - VERIFY implemented
- ⚠️ OTC medicine approval (post-2019) - UPDATE rules

### Batch 11-12: Tax Reporting
**Scan:**
- IRS form generation (Form 8889, Form 1099-SA, Form W-2 Box 12)
...
**Regulatory Focus:**
- HIPAA §164.312 (access control, audit, MFA, encryption)
...
**Regulatory Focus:**
- PCI-DSS v4.0.1 Requirements 3-4 (protect CHD, encrypt transmission)
- PCI-DSS Requirement 10 (audit logging)

**Scope Decision:**
- ⏳ Determine if system supports card transactions (FSA/HSA debit cards)
- If YES: VALIDATE PCI-DSS compliance (PAN encryption, CVV prohibition, audit trail)
...
|--------|-----|-------------------|---------------|
| **IRS Publication 969 (2024)** | https://www.irs.gov/publications/p969 | FSA/HSA/HRA contribution limits, carryover rules, qualified expenses | Dec 11, 2025 |
| **IRS Publication 502 (2024)** | https://www.irs.gov/publications/p502 | Qualified medical expenses, OTC rule changes, preventive care | Dec 11, 2025 |
| **HIPAA Privacy Rule** | https://www.hhs.gov/hipaa/for-professionals/privacy/index.html | PHI definition, individual rights, covered entity requirements | Dec 11, 2025 |
| **HIPAA Security Rule** | https://www.hhs.gov/hipaa/for-professionals/security/index.html | Administrative/physical/technical safeguards, 45 CFR §164.312 | Dec 11, 2025 |
| **PCI-DSS v4.0.1** | https://www.pcisecuritystandards.org/standards/pci-dss/ | Payment card data security, 12 requirements, CHD protection | Dec 11, 2025 |
...
1. **IRS Contribution Limits** (2024/2025) - FSA/HSA/HRA annual limits, catch-up contributions, HDHP requirements
2. **Carryover Rules** - $640 FSA carryover OR 2.5-month grace (mutual exclusion), 100% HSA rollover
3. **Qualified Medical Expenses** - IRS Pub 502 list with 2019+ OTC rule changes
4. **HIPAA PHI Security** - Administrative/physical/technical safeguards for healthcare data
5. **PCI-DSS Payment Card Security** - CHD protection requirements (if card transactions supported)

**Next Step:** Create learning library entry in `cortex-brain/learning/healthcare-reimbursement/` (Batch 2.5 Task 7)
...
**Tasks Completed:**
1. ✅ IRS Publication 969 (HSA/FSA/HRA rules)
2. ✅ IRS Publication 502 (Qualified Medical Expenses)
3. ✅ 26 CFR §125 rules (captured in Pub 969)
4. ✅ HIPAA PHI requirements (HHS.gov)
5. ✅ PCI-DSS standards (PCIsecuritystandards.org)
6. ✅ External intelligence matrix (this document)
7. ⏳ Learning library capture (next)

**Key Findings:**
- **P0 Gap Confirmed:** No IRS contribution limit validation found (AST Batch 1)
- **Regulatory Landscape:** 3 compliance domains (IRS tax code, HIPAA security, PCI-DSS)
...
- **Code Validation:** 8 functional areas to scan (contributions, carryover, claims, reporting, security, card transactions)

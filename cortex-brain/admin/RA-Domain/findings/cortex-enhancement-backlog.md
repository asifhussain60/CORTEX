...
CORTEX currently provides **Python-specific AST analysis** using the `ast` module. The Reimbursement Accounts (RA) domain analysis exposed critical gaps when analyzing **C#/.NET codebases**:
...
erDiagram
    ...
    Member ||--o{ ReimbursementAccount : "owns"
    ReimbursementAccount ||--o{ Transaction : "contains"
...
**Sources:**
1. **Fluent API:** `modelBuilder.Entity<Employer>().HasMany(e => e.Members)`
2. **Data Annotations:** `[ForeignKey("EmployerId")]`
3. **DbContext:** `DbSet<Employer>` properties
...
```json
{
  "codebase": "ReimbursementAccounts",

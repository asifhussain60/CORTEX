...
| Issue Type | Detection Pattern | Example |
|------------|------------------|---------|
| **N+1 Query** | DB query inside loop | `foreach(account in accounts) { db.Claims.Where(...) }` |
...
### P0 Risks in Carry Over Processing
...
### P1 Risks in Carry Over Processing
- [ ] N+1 queries when fetching claim data for accounts
...
### P2 Risks in Carry Over Processing

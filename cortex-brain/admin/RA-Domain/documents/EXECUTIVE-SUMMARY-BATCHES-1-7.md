# Reimbursement Accounts Platform: Executive Analysis Summary
...
HealthEquity's Reimbursement Accounts platform manages healthcare spending accounts (FSA, HSA, HRA, Dependent Care FSA) for employers and members. The platform processes account balances, claims, year-end carryovers, and statement generation with regulatory compliance requirements from the IRS, HIPAA, and PCI-DSS.
...
### Application Scope
- **Account Types:** FSA, HSA, HRA, Dependent Care FSA
- **Users:** Employers (account sponsors), Members (account holders)
- **Core Functions:** Claims processing, balance management, card transactions, year-end carryover
...
### Project Structure
- **5 Applications:** NServiceBus endpoints, background jobs (CarryOver, FlexPlan, PercentPlanLedger)
- **2 Domain Libraries:** Employer.Domain (5 entities), Member.Domain (25 entities, 11 services)
...
1. **🔄 Carry Over Processing** *(Priority 1 - Regulatory Compliance)*
   - Transfers unused FSA funds to next year (max $640 per IRS regulations)
   - Processes 100% HSA rollovers (unlimited, no expiration)
   - V2 batch processing: 1,000 accounts/batch, 10 concurrent operations (85% faster than V1)

2. **💰 Claims Processing**
   - Manual claims: Member submits receipt → admin approves → reimburse
   - Auto-Pay: Provider submits claim → auto-approved → payment scheduled
...
3. **💳 Balance Management**
   ...
   - Audit trail for all balance changes (HIPAA requirement)

4. **📅 Plan Year Management**
   - Lifecycle: Pre-Open → Open Enrollment → Active → Year-End → Closed
...
6-14. *Additional capabilities: Flex Plan Processing, Percent Plan Ledger, Carryover Transfer Tracking, Scheduled Items, Card Transactions, Cash In/Out, Dependent Management, Rollover Settings, Global Contribution Limits*

### 7 Core Business Workflows
1. Member Enrollment
2. Contribution Processing (employer deposits)
3. Claims Submission (manual + auto-pay)
4. Card Transaction Processing
5. **Year-End Carry Over** *(Critical regulatory workflow)*
...
### IRS Tax Code (Healthcare Account Regulations)

**2024-2025 Contribution Limits:**
| Plan Type | 2024 Limit | 2025 Limit | Carryover Rule |
|-----------|------------|------------|----------------|
| **FSA** | $3,200 | TBD (Nov 2025) | **$640 max OR 2.5-month grace** *(mutual exclusion)* |
| **HSA Individual** | $4,150 | $4,300 | **100% unlimited rollover** |
| **HSA Family** | $8,300 | $8,550 | **100% unlimited rollover** |
| **HSA Catch-up (55+)** | $1,000 | $1,000 | N/A |
| **Dependent Care FSA** | $5,000 | $5,000 | **None (use-it-or-lose-it)** |

**Sources:** IRS Publication 969, IRC §223, 26 CFR §125-5

### HIPAA Security Standards (PHI Protection)
...
- **PHI Examples:** Account balances, claims data, transaction history, member demographics
...
|--------|-------------|------------|---------------|--------|
| **P0-001** | No FSA $3,200 annual limit validation | IRS Pub 969 | Over-contribution tax violations, IRS penalties | High |
| **P0-002** | No HSA $4,150/$8,300 annual limit validation | IRC §223 | IRS penalties ($50/mo for excess contributions) | High |
| **P0-003** | No FSA $640 carryover limit enforcement | 26 CFR §125-5 | Regulatory non-compliance, plan disqualification | Critical |
| **P0-004** | Dependent Care FSA allows carryover (should be $0) | IRS Pub 969 | Tax-advantaged status at risk | Critical |
| **P0-005** | Cosmetic surgery not rejected as unqualified expense | IRS Pub 502 | Improper tax-free reimbursements | Medium |
| **P0-006** | CVV storage prohibition not validated | PCI-DSS Req 3.2 | Payment card data breach, $5-25k/month fines | Critical |
| **P0-007** | PAN encryption at rest not validated | PCI-DSS Req 3.4 | Cardholder data exposure, compliance violation | Critical |
| **P0-008** | PHI access audit trail completeness | HIPAA §164.312(b) | OCR investigation, $100-$50k per violation | High |
| **P0-009** | MFA enforcement for PHI access | HIPAA §164.312(a) | Unauthorized access, data breach notification costs | High |

**Estimated Compliance Risk:** $500,000 - $2,000,000 potential exposure (IRS penalties, PCI-DSS fines, HIPAA violations)
...
## 🔍 Carryover Logic Deep Dive

### Architecture Overview

**Complete Workflow Chain:**
```
CarryOver.Jobs (scheduled)
    ↓
CarryoverDollarsDomainService (orchestration)
    ↓
Entity Layer (CarryoverTransferTracking, RolloverSettings, GlobalContributionMaxByYear)
    ↓
...
**MAIN ENTRY POINT:**  
`CalculateForefeitAndCarryoverBalanceEOFYAllEmployersIdAsync` (Line 398)
...
- Applies plan-type-specific rules:
  - **FSA:** Maximum $640 (2025 IRS limit)
  - **HSA:** 100% unlimited rollover
  - **HRA:** Employer-defined
  - **Dependent Care FSA:** $0 (strict use-it-or-lose-it)
...
### Carryover Rules by Plan Type

| Plan Type | Carryover Limit | Forfeiture | Who Owns Forfeiture |
|-----------|----------------|------------|---------------------|
| **FSA** | $640 (2025, indexed annually) | Balance > $640 | Employer |
| **HSA** | 100% unlimited | None | N/A (employee-owned) |
| **HRA** | Employer-defined | Per plan document | Employer |
| **Dependent Care FSA** | $0 (use-it-or-lose-it) | 100% of balance | Employer |

**Grace Period Alternative:** FSA can offer 2.5-month grace period OR $640 carryover, but NOT both (26 CFR §125-5).

### Integration Points

**NServiceBus Events:**
- **Published:** BalanceChangedEvent (after carryover calculation)
- **Consumed:** PlanYearEndEvent (triggers carryover), ClaimSettlementEvent (updates balance), GracePeriodExpirationEvent (triggers forfeiture)

**Database Entities:**
- ReimbursementAccount (account master)
- CarryoverTransferTrackingEntity (audit trail)
- RolloverSettings (plan-specific rules)
- GlobalContributionMaxByYear (IRS limits by year)
...
- Pre-fetch claim data (reduce DB queries)
...
- **Business Impact:** Member complaints, support ticket volume
...
### Account Types
- **FSA (Flexible Spending Account):** Pre-tax funds, $3,200 limit, use-it-or-lose-it (with $640 carryover option)
- **HSA (Health Savings Account):** Triple tax-advantaged, $4,150/$8,300 limit, 100% rollover, employee-owned
- **HRA (Health Reimbursement Arrangement):** Employer-funded, employer-owned, employer-defined rules
- **Dependent Care FSA:** Pre-tax funds for daycare, $5,000 limit, strict use-it-or-lose-it (no carryover)

### Key Terms
- **EOFY (End of Fiscal Year):** Trigger for carryover processing
- **Forfeiture:** Unused funds returned to employer (FSA/HRA)
- **Rollover:** Unused funds transferred to next year
- **Grace Period:** 2.5-month extension to use prior year funds (alternative to carryover)
- **Qualified Medical Expense:** IRS-approved expense eligible for tax-free reimbursement (IRS Pub 502)
- **Auto-Pay:** Provider-submitted claims auto-approved without member action
- **PHI (Protected Health Information):** HIPAA-protected data (balances, claims, transactions)
- **PAN (Primary Account Number):** Debit/credit card number requiring PCI-DSS protection

### HealthEquity-Specific Terms
- **Plan Year:** Enrollment period (typically Jan 1 - Dec 31)
- **Reimbursement Account:** Generic term for FSA/HSA/HRA accounts
- **Card Transaction:** FSA/HSA debit card usage at point-of-sale
...
- Hqy.Employer.Domain.Tests (unit tests)
- Hqy.Member.Domain.Tests (unit tests)
...
- Regulatory scenario coverage (IRS limits, carryover rules)
...
- Carryover/forfeiture calculations
- Grace period vs. carryover mutual exclusion
...
| **2.5** | 60 min | ✅ COMPLETE | External regulatory intelligence (IRS/HIPAA/PCI-DSS) |
...
### Strengths
1. **Well-Architected:** Domain-driven design with clear bounded contexts (Employer vs. Member)
...
### Weaknesses
1. **Compliance Gaps:** 9 P0 issues (IRS limit validation, HIPAA audit, PCI-DSS security)
...
5. **Legacy Code:** V1 carryover logic still present (tech debt)

### Opportunities
1. **Regulatory Automation:** Add automated IRS limit validation with annual updates
2. **Enhanced Audit:** Complete PHI access logging for HIPAA compliance
3. **PCI-DSS Hardening:** Validate CVV prohibition, PAN encryption
...
### Threats
1. **IRS Non-Compliance:** Penalties for over-contribution, improper carryover
2. **HIPAA Violations:** OCR investigation, $100-$50k per violation
3. **PCI-DSS Fines:** $5-25k/month for non-compliance
4. **Data Breach:** Exposure of PHI or payment card data
5. **Regulatory Changes:** Annual IRS limit updates require code changes
...
3. **IRS Limit Validation:** Add automated checks for FSA/HSA contribution limits
4. **PCI-DSS Audit:** Verify CVV prohibition and PAN encryption implementation
5. **HIPAA Audit Trail:** Confirm complete PHI access logging

### Short-Term (P1 - Next Quarter)
6. **Test Coverage:** Execute 40+ regulatory test scenarios identified
7. **Grace Period Logic:** Validate mutual exclusion of carryover and grace period
8. **Dependent Care FSA:** Confirm zero carryover enforcement
...
11. **Automated Compliance:** Annual IRS limit updates via configuration (no code changes)
...
- `discovery/carryover-logic-investigation.md` - Carry over business rules
...
- `ast-outputs/carryover-service-methods.json` - Complete method signatures

### B. External References
- IRS Publication 969 (2024): https://www.irs.gov/pub/irs-pdf/p969.pdf
- IRS Publication 502 (2024): https://www.irs.gov/pub/irs-pdf/p502.pdf
...
- **External Research:** Authoritative sources (IRS.gov, HHS.gov, PCIsecuritystandards.org)

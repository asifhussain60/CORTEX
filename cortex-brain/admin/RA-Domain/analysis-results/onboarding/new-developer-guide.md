...
- ✅ Understand the RA domain (FSA, HSA, HRA, Carryover)
...
**What to Read:**
- `terminology-guide.md` - 80+ business terms (FSA, HSA, carryover, etc.)
- IRS Publication 969 (Health Savings Accounts)
- Internal wiki: "Reimbursement Accounts 101"

**Key Concepts:**
- **FSA (Flexible Spending Account):** Pre-tax healthcare funds, $3,200 annual limit
- **HSA (Health Savings Account):** Tax-advantaged savings, $4,150 individual limit
- **HRA (Health Reimbursement Arrangement):** Employer-funded only
- **Carryover:** FSA $640 max, HSA unlimited
- **Qualified Expenses:** IRS Pub 502 list

**Quiz Questions:**
1. What's the difference between FSA and HSA?
2. How much can FSA participants carry over?
...
### Step 3: Deep Dive - Carryover Logic (75 min)

**Why Carryover First?**
- Most complex business logic (717 LOC, 20+ methods)
- Highest regulatory risk ($500k IRS penalties)
- Frequently changes (annual IRS limit updates)
...
```csharp
/// <summary>
/// Calculates FSA carryover amount based on IRS limits ($640 max for 2025)
/// </summary>
/// <param name="plan">FSA plan with carryover enabled</param>
/// <param name="remainingBalance">Unused balance from prior year</param>
/// <returns>Carryover amount (capped at IRS limit)</returns>
/// <exception cref="InvalidOperationException">Thrown if plan doesn't allow carryover</exception>
...
public void CalculateCarryoverAmount_WhenBalanceExceedsLimit_ReturnsIrsMax()
{
    ...
    // Assert
    Assert.Equal(640m, carryoverAmount); // IRS 2025 limit
}
```
...
| Carryover logic | [Senior Dev - Carryover SME] | #ra-engineering |

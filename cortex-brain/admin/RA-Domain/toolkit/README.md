...
**Purpose:** Deep domain analysis for Product.ReimbursementAccounts repository
...
# Repository access
Test-Path "C:\PROJECTS\Product.ReimbursementAccounts"  # Should return True
```

### Quick Scan (30 seconds)
```powershell
# Structural analysis only
.\scripts\01_quick_scan.ps1 -RepoPath "C:\PROJECTS\Product.ReimbursementAccounts"
```

### Full Analysis (17.58 hours - batched execution)
```powershell
# Execute all 20 batches
python scripts\06_master_orchestrator.py --repo-path "C:\PROJECTS\Product.ReimbursementAccounts" --execute-all
```
...
python scripts\03_business_logic_extractor.py --focus-service CarryoverDollarsDomainService
```
...
│   ├── regulatory-rules.json           # IRS/HIPAA/PCI-DSS rules
...
│       ├── irs_validator.py            # IRS compliance
│       ├── hipaa_validator.py          # HIPAA audit
│       └── pci_dss_validator.py        # PCI-DSS checks
│
└── tests/                              # Unit tests
    ├── test_collectors.py
    ├── test_parsers.py
...
- **Scenario Generation:** 40+ regulatory test cases (IRS/HIPAA/PCI-DSS)

### 3. Regulatory Compliance Validation
- **IRS Tax Code:** FSA/HSA contribution limits, carryover rules, grace period
- **HIPAA Security:** PHI audit trails, access controls, MFA enforcement
- **PCI-DSS:** CVV prohibition, PAN encryption, card masking

### 4. Business Logic Extraction
- **Decision Trees:** Plan-type-specific rules (FSA/HSA/HRA/Dependent Care)
- **Workflow Definitions:** 7 core business workflows
- **Simplified Explanations:** Carryover logic, claims processing, balance management
...
# Review: business-logic/carryover-rules-simplified.md
```
...
```powershell
# Verify path
Test-Path "C:\PROJECTS\Product.ReimbursementAccounts"
...
- **Regulatory Rules:** `config/regulatory-rules.json` - IRS/HIPAA/PCI-DSS requirements

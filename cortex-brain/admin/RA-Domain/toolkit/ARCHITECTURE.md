...
│  │  - EntityCollector │  - CoverageCollector│  - IRSValidator │ │
│  │  - DTOCollector    │  - TestMapCollector│  - HIPAAChecker │ │
...
class RABaseCollector(BaseDataCollector):
    ...
    def _validate_compliance(self, entity: Dict) -> List[str]:
        """NEW: Validate against IRS/HIPAA/PCI-DSS rules"""
        pass
```
    ...
    def collect_batch(self, start_idx: int, end_idx: int) -> Dict[str, Any]:
        ...
        for file in self._get_entity_files()[start_idx:end_idx]:
            entity_data = {
                ...
                'compliance_flags': self._check_phi_pci_fields(file)  # HIPAA/PCI
            }
...
class ServiceCollector(RABaseCollector):
    ...
    Special Focus: CarryoverDollarsDomainService (20 methods)
    """
...
```python
class IRSComplianceValidator(RABaseCollector):
    """
    Validate IRS tax code compliance
    
    Batch: 2.5 + ongoing validation
    Output: regulatory/irs-compliance-report.json
    
    Checks:
    - Contribution limits (FSA: $3,200, HSA: $4,150/$8,300)
    - Carryover limits (FSA: $640 max, HSA: unlimited)
...
```
cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts/
...
│   └── carryover-service-methods.json  # Deep dive
│
...
│   ├── irs-compliance-report.json      # IRS Pub 969, IRC §223
│   ├── hipaa-audit-findings.json       # HIPAA §164.312
│   ├── pci-dss-scan-results.json       # PCI-DSS Req 3.2/3.4
│   └── p0-issues-tracking.json         # Critical gaps
│
├── business-logic/                     # Extracted business rules
│   ├── carryover-rules-simplified.md   # Decision trees
│   ├── plan-type-matrix.json           # FSA/HSA/HRA/Dependent Care
...
def discover_and_register_ra_domain():
    ...
    """
    ra_domain_path = Path('C:/PROJECTS/Product.ReimbursementAccounts')
    output_path = Path('cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts')
    
    # Check if analysis exists
    if (output_path / 'dashboard.json').exists():
        registry_entry = {
            'id': 'ra-domain-analysis',
            'name': 'Product.ReimbursementAccounts',
...
.\01_quick_scan.ps1 -RepoPath "C:\PROJECTS\Product.ReimbursementAccounts"

# Full analysis (all 20 batches)
python 06_master_orchestrator.py --repo-path "C:\PROJECTS\Product.ReimbursementAccounts" --execute-all

# Specific batch execution
python 06_master_orchestrator.py --repo-path "C:\PROJECTS\Product.ReimbursementAccounts" --batch 11
```
...
python -m src.orchestrators.dashboard_launcher --source "Product.ReimbursementAccounts"
...
orchestrator = RADomainOrchestrator(
    repo_path='C:/PROJECTS/Product.ReimbursementAccounts',
    output_dir='cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts'
)
...
repository:
  path: "C:/PROJECTS/Product.ReimbursementAccounts"
  name: "Product.ReimbursementAccounts"
  type: "external_csharp"

output:
  base_dir: "cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts"
  formats:
    - json
    - markdown
...
- **Rationale:** Healthcare domain requires IRS/HIPAA/PCI-DSS compliance
    ...
    def generate_onboarding_package(self) -> Dict[str, Any]:
        return {
            'learning_path': [
                ...
                {'order': 2, 'topic': 'Domain Model (FSA/HSA/HRA)', 'time': '3 hrs'},
                {'order': 3, 'topic': 'CarryoverDollarsDomainService', 'time': '4 hrs'},
...
class TestCoverageIntelligenceCollector(RABaseCollector):
    ...
    - Ranks by regulatory risk (IRS/HIPAA violations)
    ...
    def prioritize_test_gaps(self) -> List[Dict]:
        ...
        1. Regulatory risk (IRS carryover = P0)
        ...
        # P0: Carryover logic (IRS compliance risk)
        gaps.append({
            'priority': 'P0',
            'component': 'CarryoverDollarsDomainService',
            'risk': 'IRS non-compliance, $500k penalty exposure',
            'current_coverage': '0%',
            'target_coverage': '100%',
        ...
        gaps.append({
            'priority': 'P0',
            'component': 'ReimbursementAccountBalanceService',
            'risk': 'Incorrect member balances, refund liability',
            'current_coverage': '10%',
            'target_coverage': '95%',
...
- **Sprint 1-2:** P0 regulatory scenarios (Carryover, Balance) → 30% coverage
- **Sprint 3-4:** P1 financial calculations (Claims, Card) → 60% coverage
...
class BusinessFunctionCatalogCollector(RABaseCollector):
    ...
    def generate_catalog(self) -> Dict[str, Any]:
        return {
            'capabilities': [
                {
                    'name': 'Year-End Carryover Processing',
                    'services': ['CarryoverDollarsDomainService', 'CarryoverSharedService'],
                    'background_jobs': ['CarryOver.Jobs'],
                    'domain_events': ['BalanceChangedEvent', 'PlanYearEndEvent'],
                    'test_coverage': '8%',  # CRITICAL GAP
                    'complexity': 'HIGH (717 LOC)',
                    'regulatory_compliance': 'IRS Pub 969, IRC §223',
                    'product_alignment': 'Q1 2025 Roadmap - Automated Carryover v2'
                },
                {
                    'name': 'Claims Processing',
                    'services': ['ReimbursementAccountBalanceService'],
                    'background_jobs': [],
                    'test_coverage': '10%',
    ...
    def generate_documentation(self) -> Dict[str, Any]:
        return {
            ...
            'missing_xml_comments': [
                'CarryoverDollarsDomainService (20 methods undocumented)',
                'ReimbursementAccountBalanceService (15 methods undocumented)'
            ],
            'quality_gates': {
...
| **Bus Factor** | 1 (Carryover) | 2+ | ⚠️ CRITICAL | Cross-train by EOQ |
...
│   └── business-glossary.html          # 80+ terms (FSA, HSA, carryover, etc.)
│
├── regulatory/                         # Compliance views
│   ├── compliance-status.html          # IRS/HIPAA/PCI-DSS dashboard
...
<body>
    <header>
        <h1>🏥 Reimbursement Accounts Domain Analysis</h1>
...
# 2. Output: cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts/onedrive-site/
...
2. Create `product-reimbursement-accounts.json` matching admin dashboard schema
...
2. Auto-update `cortex-brain/dashboards/data/repos/Product.ReimbursementAccounts/latest.json`
...
**Solution:** Repository registry pattern makes adding future repos trivial (e.g., Product.Claims, Product.Eligibility)

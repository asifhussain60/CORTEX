...
## 🔄 View Modification Strategy (Delete & Recreate)

### ⛔ PREFER DELETE + RECREATE over Complex Modifications

**When modifications to an existing view become complex, DELETE the file and RECREATE it from scratch.**
...
| Approach | Risk | Time | Quality |
|----------|------|------|---------|
| **Complex patch** | High (broken HTML/CSS) | Longer | Inconsistent |
| **Delete + Recreate** | Low (clean slate) | Faster | Consistent |

### Workflow for Delete + Recreate

```bash
# 1. Backup existing file (automatic via git)
git status  # Ensure clean working state

# 2. Delete the file
rm docs/{section}/{page}.html

# 3. Regenerate using design standards
# Follow glassmorphism-design-standards-v2.md from scratch

# 4. Validate new file
python cortex-toolkit/documentation/design_validator.py --path docs/{section}/{page}.html
```

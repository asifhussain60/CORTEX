        ...
        grid.innerHTML = data.tiers.map(tier => `
            <div class="glass-panel cursor-pointer hover:border-cyan-500/50 transition group">
                ...
                <div class="mt-4 pt-4 border-t border-slate-700/50 opacity-0 group-hover:opacity-100 transition">
                    <button class="text-xs text-cyan-400 hover:text-cyan-300">View Details ΓåÆ</button>
                </div>
            </div>
        `).join('');
        
        console.log('Γ£ô Brain tiers rendered:', data.tiers.length);
        
    } catch (error) {
        console.error('Γ£ù Error loading brain tiers:', error);
        grid.innerHTML = `
            <div class="col-span-full glass-panel p-4 text-amber-400 border-l-2 border-amber-500">
                <p class="text-sm">ΓÜá∩╕Å Unable to load brain architecture</p>

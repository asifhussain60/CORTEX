...
### Gap Detection & Tracking (NEW - Phase-22 MCP Focus)
- `/gaps` - Show all design-build gaps in current inventory
- `/gaps <phase>` - Show gaps in specific phase
- `/gap-status <ac-id>` - Show exposure/integration status
- `/gap-remediation` - Show recommended remediations (new phases/ACs)
- `/gap-audit` - Quarterly gap audit across all phases
...
progress_report:
  ...
  gaps_summary:
    total_design_build_gaps: 8
    critical_gaps: 1
    gap_inventory:
      - gap_id: "MCP-001"
        severity: "CRITICAL"
        component: "MCP Server"
        status: "DESIGNED_NOT_EXPOSED"
        remediation: "Phase-22-MCP-Protocol-Compliance (8 ACs)"
      - gap_id: "CONFIG-001"
        severity: "HIGH"
        component: "Client Configurations"
        status: "DESIGNED_NOT_IMPLEMENTED"
        remediation: "AC-MCP-003-01 (Claude Desktop config)"

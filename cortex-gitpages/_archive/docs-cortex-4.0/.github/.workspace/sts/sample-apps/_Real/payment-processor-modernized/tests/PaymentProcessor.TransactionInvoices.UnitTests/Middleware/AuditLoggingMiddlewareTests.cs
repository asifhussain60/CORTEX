...
namespace PaymentProcessor.TransactionInvoices.UnitTests.Middleware;
...
public class AuditLoggingMiddlewareTests
{
    ...
    public AuditLoggingMiddlewareTests()
    {
        ...
        _mockConfiguration.Setup(c => c["AuditLogging:RedactPII"]).Returns("true");
    }
    ...
    public async Task InvokeAsync_ShouldRedactSSN_WhenEnabled()
    {
        ...
        var context = CreateHttpContext("POST", "/api/customers", requestBody);
    ...
    [Fact]
    public async Task InvokeAsync_ShouldRedactCustomerName_WhenEnabled()
    {
        // Arrange
        var requestBody = "{\"customerName\": \"John Doe\"}";
        var context = CreateHttpContext("POST", "/api/account_categorys", requestBody);

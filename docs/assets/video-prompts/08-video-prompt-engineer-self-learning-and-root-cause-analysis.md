...
- Knowledge Acquisition Layer: `KnowledgeAcquisitionOrchestrator` (`cortex/intelligence/knowledge/knowledge_acquisition_orchestrator.py`) is a companion self-improvement mechanism that operates at the knowledge level rather than the pattern level. It runs a 6-stage pipeline — signal extraction, coverage assessment, template synthesis, schema validation, index registration, and cache invalidation — and triggers automatically when LENS domain signal coverage falls below 80% for any active domain. On completion, it emits a reinforcement reward into the universal learning loop. The schema validator enforces 7 structural rules to ensure synthesised knowledge files are coherent before registration. Do NOT describe this as replacing the reinforcement loop; it runs alongside it, addressing structural knowledge gaps the signal loop cannot fill on its own. Key supporting files: `cortex/intelligence/knowledge/domain_signal_extractor.py`, `cortex/intelligence/knowledge/knowledge_coverage_assessor.py`

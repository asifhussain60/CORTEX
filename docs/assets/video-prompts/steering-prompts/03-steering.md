# Steering Prompt — Video 03: CORTEX for Engineering & Quality
## NotebookLM Cinematic Video — Setup Guide
**Source:** `sources/03-engineering-quality-source.md` | **Format:** Cinematic | **Narrator:** Male
...
| Content sources | `docs/.content/02-intelligence-how-cortex-understands-code.md` · `docs/.content/04-tdd-quality-flywheel.md` · `docs/.content/05-orchestration-at-scale.md` | ✅ Engineering propositions synthesised |
...
- [ ] Create new notebook
- [ ] Upload `sources/03-engineering-quality-source.md` as the **only** source
...
- [ ] If quality dimension is underweighted → use Fallback prompt below
...
```
Cinematic 6–8 minute video for software and quality engineers. Open with AI
generating code at zero context — CI fails, errors flood terminal. CORTEX
activates: LENS pipeline illuminates layer by layer, context fills to 100%
from 140+ knowledge files across 15 engineering domains.

Show three capabilities in sequence:
1. TDD as a glowing neon loop (RED→GREEN→REFACTOR), enforced as infrastructure
   ...
   (Phase 142) means every workflow step is composed, not repeated. WorkflowGateway
   as the single SSOT for all step definitions, eliminating fork drift.
2. Quality radar chart filling five dimensions to a composite score. Response
   Rendering Rules (Phase 146) validate all 14 response formatting constraints
   before output. Golden test card traces back to its acceptance criterion —
   the business rule is always the proof.
3. Convergence gate dropping violations from 15 to zero — detect, fix, rescan
   loop (CORE-068) stamping SWEEP COMPLETE in gold. Not a one-pass check.

Male narrator, peer-to-peer tone. Engineers should feel understood.
```

---

## 🔄 Fallback Prompt (if engineering and quality are not equally weighted)

```
Create a 6–8 minute cinematic video for software and quality engineers.
Cover equally: LENS providing full codebase context before any keystroke
from 140+ knowledge files across 15 engineering domains, TDD enforced as
infrastructure (RED→GREEN→REFACTOR via CORE-008) with WorkflowGateway as
the single SSOT eliminating step-definition drift (Phase 142), 5-dimension
quality scoring with composite radar chart and 14-rule Response Rendering
validation (Phase 146), golden tests protecting critical business rules
with acceptance-criterion traceability, and convergence gate sweeping all
issues to zero with a SWEEP COMPLETE gold seal (CORE-068). Cyan and gold
accents. Male narrator. Peer-to-peer technical tone — respect the craft.
```

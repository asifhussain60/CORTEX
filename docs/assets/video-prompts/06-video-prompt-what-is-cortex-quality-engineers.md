# Video Prompt 06 — CORTEX for Quality Engineers

---
**Series:** CORTEX — The Governed AI Engineering Partner
**Video:** 06 of 07 (Role Series)
**Title:** CORTEX for Quality Engineers
**Subtitle:** Golden Tests. Convergence. Quality as Code.
**Audience:** QA Engineers, SDET, Test Leads — professionals who define what "done" means and prove it
**Duration:** 7–10 minutes
**Narrator:** 🎙️ Male — meticulous, quality-obsessed, evidence-driven; speaks with the precision of a test assertion
**Generator:** Google Gemini Video Generator / NotebookLM Video Editor
**Domain Color:** Gold `#FFD700` (golden tests, premium quality, trust)
...
**Visual:** `#0a0e27` navy. Gold particles shimmer like coins falling. Logo with gold glow. Title: "CORTEX for Quality Engineers" → subtitle: "Golden Tests. Convergence. Quality as Code." Logo shrinks to watermark.

### Scene 1 — The Quality Crisis (90s) [VBP-002 hook, VBP-006 contrast]
**Visual:** Test pyramid — but inverted (too many E2E, almost no unit tests). Flaky test icons blink orange. A test suite runs: 847 pass, 23 fail, 156 skip. A "Test Health" gauge wobbles in the red zone. Coverage badge shows "34%". An engineer mutters: "Just skip it."
**Narration:** Your test suite has thousands of tests. But how many actually prove the right thing? Flaky tests erode trust. Skipped tests hide risk. Coverage metrics lie when tests assert nothing meaningful. The real question is not "how many tests?" — it is "how much confidence do your tests create?"
...
**Visual:** Convergence gate loop — detect (magnifying glass scans code)→fix (wrench applies patch)→rescan (check mark validates). Loop runs 3 times. A sweep catalogue sidebar shows 15 issues — each flips from OPEN to CLOSED. Counter: "15→8→2→0". Final gate opens with gold light. A "SWEEP COMPLETE" seal stamps with authority.
**Narration:** The convergence gate is quality's enforcement mechanism. It does not pass on the first scan — it loops. Detect, fix, rescan — until zero violations remain. Sweep completeness means every issue found must be resolved. No cherry-picking the easy three and leaving twelve behind. Partial fixes are governance violations. Your test suite is either fully healthy or transparently in progress.
**Animation:** Each loop cycle is visually distinct — ring 1 (red), ring 2 (amber), ring 3 (green). Sweep catalogue items animate like a slot machine resolving to "CLOSED". Final counter: "0 remaining" in gold. Seal animation with confetti. [VBP-016: bold **convergence**, **sweep**, **zero violations**]
...
| VBP-002 Hook in 8s | ✅ Inverted pyramid, flaky tests |
| VBP-003 Narration ≠ slide | ✅ Radar chart complements scoring narration |
| VBP-006 Contrast | ✅ Broken pyramid → corrected pyramid |
| VBP-007 2-min cycles | ✅ Scene transitions every 90–120s |
| VBP-009 Signaling | ✅ Gold glow on active dimension |
| VBP-011 Strategic Silence | ✅ After "starts knowing" |
| VBP-012 Consistent visuals | ✅ Gold domain accent throughout |
| VBP-013 Business book anchoring | ✅ Deming quote |
| VBP-014 Hero intro | ✅ 5s gold shimmer |
| VBP-015 Breadcrumb | ✅ 3-stage progress bar |
| VBP-016 Bold keywords | ✅ Gold on quality terms |
| VBP-017 Male narrator | ✅ Even-numbered |
| VBP-018 Acronyms expanded | ✅ TDD, QA, SDET, AC |
| VBP-019 Colour intelligence | ✅ Gold for quality, red/green for TDD |

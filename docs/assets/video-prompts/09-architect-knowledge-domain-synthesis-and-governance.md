...
- 60+ governance YAMLs in `cortex-registry/core/` — enforced at pre-commit, CI, and runtime
...
- Knowledge resolution priority (5-tier — `KnowledgeGuidanceEngine`): When multiple knowledge sources could apply, resolution follows a strict precedence chain — (1) DOMAIN_OVERRIDE: any `__override__: true` entry in a domain profile, highest priority; (2) TIER_0: skull rules, the absolute inviolable constraints; (3) TIER_1: domain-specific knowledge for the active domain profile; (4) TIER_2: cross-domain engineering knowledge; (5) CORTEX_BEST_PRACTICES: general best practices, lowest priority. This 5-tier model replaced an earlier 3-tier model. Do NOT describe it as 3-tier or 4-tier — the current implemented model has 5 tiers.
- Domain signal map: `cortex-registry/config/domain-signal-map.yaml` — maps domain identifiers to their signal extraction patterns. Updated by `KnowledgeAcquisitionOrchestrator` when auto-synthesis generates new domain knowledge files. Architects can inspect this file to understand which domains are actively monitored.
...
Visual: The 60+ governance YAML files from `cortex-registry/core/` materialise as a catalogued grid — 60+ labelled cards, colour-coded by severity: critical (red border), high (amber border), medium (yellow border), low (grey border).
...
Lower-third: `"60+ governance rules — enforced at pre-commit, continuous integration, and runtime"`

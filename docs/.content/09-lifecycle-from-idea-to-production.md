...
The **CORTEX Autonomous Planning Engine (CAPE)** handles non-trivial requests through an 11-component pipeline. First, the Complexity Triage Engine scores every request on five dimensions using the CDR (CORTEX Definition of Ready) model: clarity, context, scope, risk, and precedent — producing a composite score that routes the request into one of three bands (SIMPLE, MODERATE, or COMPLEX). COMPLEX requests trigger full plan generation via Kahn's topological dependency ordering algorithm. The Auto-Plan Generator writes a structured YAML phase plan to the planning registry, including a sweep catalogue, TDD red→green→refactor cycles, and acceptance criteria. Before any plan executes, five analysis gates fire in sequence: threat modelling (STRIDE/DREAD), quality scoring (8 dimensions), security assessment (OWASP Top 10), RCA history consultation, and Operational Pattern Journal review. A seven-item machine-checkable completion checklist must pass before any phase is marked complete.

For large initiatives, wave-based delivery planning produces a sequence of increments each independently deliverable and valuable. Per-sub-phase convergence cleanup (3-cycle max, CORE-068) and a holistic stabilisation phase with 9 checks ensure that plan execution leaves the codebase cleaner than it found it.

...
## DESIGN-BUILD GAP VERIFICATION (NEW - Phase-22 MCP Focus)

**Purpose:** Verify that components are properly exposed and integrated, not just designed and tested.

### Gap Detection Queries

```sql
-- 1. Find ACs that are COMPLETE in cortex-master.yaml but not MCP-exposed
SELECT 
  ac_id,
  'MCP_NOT_EXPOSED' as gap_type,
  'Component implemented but @mcp_tool decorator missing' as issue
FROM audit_log
WHERE 
  ac_id IN (
  ...
  AND ac_id NOT IN (
    SELECT ac_id FROM mcp_exposure_log 
    WHERE decorator_present = 1
  )
GROUP BY ac_id;

-- 2. Find components with 100% passing tests but no integration
SELECT 
  ac_id,
  'NOT_INTEGRATED' as gap_type,
  'All tests pass but component not in __all__ or MCPServer' as issue
FROM audit_log
WHERE 
  operation = 'AC_COMPLETE'
  AND ac_id NOT IN (
    SELECT ac_id FROM integration_log 
    WHERE exported = 1 AND registered = 1
  )
GROUP BY ac_id;

-- 3. Find governance enforcement gaps (rules defined but not enforced)
SELECT 
  DISTINCT core_rule_id,
  'ENFORCEMENT_GAP' as gap_type,
  'Rule defined in core-rules.yaml but not enforced in phase checks' as issue
FROM (
  SELECT 'CORE-024' as core_rule_id 
  UNION ALL
  SELECT 'CORE-027'
  UNION ALL
  SELECT 'CORE-011'
)
WHERE core_rule_id NOT IN (
  SELECT DISTINCT rule_checked_in FROM phase_enforcement_log
);
```

### Gap Detection Checklist (Per Phase)

```yaml
design_build_gap_check:
  for_each_completed_ac:
    phase_1_design:
      - Component described in phase YAML ✓
      - AC-IDs marked COMPLETED in cortex-master.yaml ✓
      - Tests show 100% passing ✓
    
    phase_2_implementation:
      - Code exists (not stubbed/TODO) ✓
      - Implementation matches design ✓
      - No blocking TODOs remain ✓
    
    phase_3_exposure:
      - ✓ Tool-eligible components have @mcp_tool decorator
      - ✓ Component exported in __all__ (Python modules)
      - ✓ Registered in MCPServer (if MCP-eligible)
      - ✓ Discovery mechanism working (tools listed in /tools endpoint)
    
    phase_4_governance:
      - ✓ Audit trail complete (AC_START, AC_EXECUTE, AC_COMPLETE in governance.db)
      - ✓ All applicable CORE rules enforced
      - ✓ Compliance validated and logged
    ...
    if_any_check_fails:
      - Report as DESIGN_BUILD_GAP
      - Suggest remediation AC-ID
      - Do NOT mark phase as complete until resolved
```
...
gap_finding:
  ...
  title: "MCP Server Not Spec-Compliant (Design-Build Gap)"
  
  details: |
    - Component: MCP Server (src/mcp_server/)
    - Design: Full MCP protocol implementation (phase-20.yaml)
    - Status: AC-MCP-001-01 through AC-MCP-008-01 marked COMPLETED
    - Tests: All 103 tests passing ✓
    - Gap: Component NOT spec-compliant per MCP v0.9.0+
      - Missing: stdio transport (only HTTP/websocket)
      - Missing: JSON-RPC 2.0 compliance
      - Missing: mcp>=0.9.0 in requirements.txt
  
  evidence:
    - AC_COMPLETE logged for AC-MCP-001-01 ✓
    - 103 tests passing ✓
    - mcp_server.py exists ✓
    - @mcp_tool decorator NOT found on server initialization ✗
    - stdio transport NOT implemented ✗
    - requirements.txt shows mcp==0.8.1 (needs 0.9.0+) ✗
  
  root_cause: |
    Design phase (Phase-20) specified MCP v0.9.0 support.
    Implementation satisfies custom HTTP interface.
    Governance check for "spec-compliant" was not enforced during AC-MCP-001-01.
    Gap not detected because no verification step in design→build→expose flow.
  
  remediation:
    - Create AC-MCP-001-02 (MCP SDK Integration - spec compliance)
    - Add pre-phase-lock verification in cortex-builder.md
    - Add quarterly gap audit to cortex-planner.md
    - Update phase-22-mcp-protocol-compliance.yaml
```

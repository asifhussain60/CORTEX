...
- [OperationModuleMetadata](#operationmodulemetadata)
- [BaseOperationModule](#baseoperationmodule)
...
- **Classes:** 6
- **Functions:** 0
- **Dependencies:** abc, dataclasses, datetime, enum, logging, pathlib, src, typing
...
- `formatted_header`: Optional[str]
- `formatted_footer`: Optional[str]
...
**Methods:**
  ...
  *Decorators:* `execution_mode.setter`
...
Example:
  ...
  **Parameters:**
  ...
  - `self`
  - `context` (Dict[str, Any]): Shared context dictionary with operation state


  **Returns:** tuple[bool, List[str]]
    Tuple of (is_valid, issues_list) - is_valid: True if prerequisites met - issues_list: List of issue descriptions (empty if valid)
...
Example:
  ...
  **Parameters:**
  ...
  - `self`
  - `context` (Dict[str, Any]): Shared context dictionary - Read context from previous modules - Write results for downstream modules - Access operation-wide configuration
...
Example:
  ...
  **Parameters:**
  ...
  - `self`
  - `context` (Dict[str, Any]): Shared context dictionary (may contain rollback hints)
...
Example:
  ...
  **Parameters:**
  ...
  - `self`
  - `user_reference` (str): File reference like "00-master-plan.md", "#file:plan.md", or full path
  - `brain_path` (Optional[Path]) = `None`: Path to cortex-brain (auto-detected if None)

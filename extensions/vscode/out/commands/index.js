...
Use @cortex in GitHub Copilot Chat for natural language interaction.
            `.trim();
        vscode.window.showInformationMessage('CORTEX Help', { modal: true, detail: helpMessage }, 'Open Documentation').then(selection => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.plan', async () => {
        ...
        if (!validation.isValid) {
            vscode.window.showErrorMessage(`CORTEX Configuration Error: ${validation.errors.join(', ')}`);
            return;
        }
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                ...
                vscode.window.showInformationMessage(`✅ Planning folder created: cortex-brain/documents/planning/active/${planName}/`);
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ Planning folder creation failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.startTdd', async () => {
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                ...
                vscode.window.showInformationMessage('✅ TDD Orchestrator started. Check CORTEX output for instructions.');
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ TDD workflow failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.systemMaintenance', async () => {
        ...
        const confirm = await vscode.window.showWarningMessage('System Maintenance will analyze and optimize your codebase. Continue?', { modal: true }, 'Yes, Run Maintenance', 'Cancel');
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                progress.report({ increment: 100, message: 'Complete!' });
                vscode.window.showInformationMessage('✅ System maintenance complete! Check CORTEX output for report.');
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ System maintenance failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.sanitize', async () => {
        ...
        // Confirm destructive operation
        const confirm = await vscode.window.showWarningMessage('Code Sanitization will remove company data and PII. This modifies files. Continue?', { modal: true }, 'Yes, Sanitize Code', 'Cancel');
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                progress.report({ increment: 100, message: 'Complete!' });
                vscode.window.showInformationMessage('✅ Code sanitization complete! Review changes before committing.');
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ Code sanitization failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.refine', async () => {
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                progress.report({ increment: 100, message: 'Complete!' });
                vscode.window.showInformationMessage('✅ System refinement complete! Check CORTEX output for recommendations.');
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ System refinement failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.onboard', async () => {
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                progress.report({ increment: 100, message: 'Complete!' });
                vscode.window.showInformationMessage('✅ Onboarding started! Check CORTEX output for interactive guide.');
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ Onboarding failed: ${result.error}`);
            }
        });
    }));
    // CORTEX: ADO Planning
    context.subscriptions.push(vscode.commands.registerCommand('cortex.adoPlanning', async () => {
        ...
        // Get work item type
        const workItemType = await vscode.window.showQuickPick(['Story', 'Feature', 'Task', 'Bug', 'Epic'], {
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                progress.report({ increment: 100, message: 'Complete!' });
                vscode.window.showInformationMessage(`✅ ADO ${workItemType} created successfully!`);
                outputChannel.show();
            }
            else {
                vscode.window.showErrorMessage(`❌ ADO planning failed: ${result.error}`);
            }
        });
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cortex.showDashboard', async () => {
        ...
        vscode.window.showInformationMessage('CORTEX Dashboard coming soon! Use individual commands or @cortex in Copilot Chat.');
    }));

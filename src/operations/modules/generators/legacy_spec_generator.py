...
class LegacySpecGenerator:
    ...
    def __init__(self, legacy_file: Path, output_dir: Path, enable_narrative_validation: bool = True):
        ...
        self.enable_narrative_validation = enable_narrative_validation  # NEW: Narrative quality validation
        
        # Import narrative validator if enabled
        if self.enable_narrative_validation:
            try:
                import sys
                from pathlib import Path as P
                toolkit_path = P(__file__).parent.parent.parent.parent.parent / 'cortex-toolkit'
                if str(toolkit_path) not in sys.path:
                    sys.path.insert(0, str(toolkit_path))
                from core.generators.narrative_validator import NarrativeValidator
                self.narrative_validator = NarrativeValidator()
            except ImportError:
                print("⚠️  Warning: Narrative validator not available, disabling")
                self.enable_narrative_validation = False
                self.narrative_validator = None
        else:
            self.narrative_validator = None
...
"""
    ...
    def _generate_business_purpose(self) -> str:
        """Generate business purpose from class name with narrative validation."""
        # Use narrative validator if available
        if self.enable_narrative_validation and self.narrative_validator:
            action = self.narrative_validator.humanize_class_name(self.class_name)
            purpose = f"{action.capitalize()} operation for RA funding management"
            return self.narrative_validator.validate_heading(purpose)
        
        # Fallback to original logic (legacy behavior)
    ...
    def generate_test_scenarios(self) -> str:
        ...
        """
        doc = f"""# Test Scenarios - {self.class_name}
...
This document outlines test scenarios for the **{self.class_name}** API, organized by priority:

- **P0 (Critical):** Core functionality that must work - business critical path
- **P1 (High):** Happy path variations and common use cases
- **P2 (Medium):** Edge cases, error handling, boundary conditions

**Test Coverage:**
- {len(self.business_rules)} Business Rules
- {len(self.validations)} Validation Rules
- {len(self.db_operations)} Database Operations
- {len(self.methods)} Methods

---

## 🔴 P0 - Critical Path Tests

These tests verify the core business functionality. **All P0 tests must pass before release.**

"""
        
        # Generate P0 tests from main business rules and critical validations
        p0_count = 1
        
        # Primary method execution (if exists)
        primary_method = next((m for m in self.methods if m.name in ['Execute', 'Run', 'Process']), None)
        if primary_method:
            doc += f"""### P0-{p0_count:03d}: Execute Primary Operation Successfully

**Objective:** Verify that the main operation executes without errors under normal conditions
...
**Test Steps:**
1. Initialize {self.class_name} with valid configuration
2. Call `{primary_method.name}()` method
...
**Legacy Reference:** Lines {primary_method.line_start}-{primary_method.line_end}
...
"""
        ...
        # Critical business rules (first 3)
        for rule in self.business_rules[:3]:
            doc += f"""### P0-{p0_count:03d}: {rule.name}

**Objective:** Verify business rule enforcement: {rule.description}

**Test Data:**
- Condition: `{rule.condition}`
- Expected Action: Execute designated business logic

**Test Steps:**
1. Set up test data to satisfy condition: `{rule.condition}`
2. Execute the operation
3. Verify the action is performed correctly

**Expected Results:**
- When condition is TRUE: Action executes as defined
- When condition is FALSE: Action is skipped
- No unexpected side effects

**Legacy Reference:** Line {rule.line_number} ({rule.layer} Layer)

**Priority Justification:** Core business logic governing system behavior
...
"""
        ...
        if self.db_operations:
            critical_ops = [op for op in self.db_operations if op.operation_type in ['INSERT', 'UPDATE', 'DELETE']][:2]
            for op in critical_ops:
                doc += f"""### P0-{p0_count:03d}: Database Operation - {op.operation_type} {op.table}

**Objective:** Verify critical database operation executes correctly
...
**Test Steps:**
1. Prepare test database with required state
2. Execute operation that triggers {op.operation_type}
3. Verify database state changes correctly
4. Check transaction rollback on failure

**Expected Results:**
- {op.operation_type} operation completes successfully
- Data integrity maintained
- Transaction boundaries respected

**Legacy Reference:** Line {op.line_number}

**Priority Justification:** Data persistence is critical to system integrity
...
These tests verify the API works correctly with different valid inputs and common scenarios.

"""
        
        # Generate P1 tests from remaining business rules
        p1_count = 1
        for rule in self.business_rules[3:6]:  # Next 3 rules
            doc += f"""### P1-{p1_count:03d}: {rule.name} - Variation Testing
...
- Input: Data satisfying `{rule.condition}`
- Expected: Action executes

**Scenario B: Condition FALSE**
- Input: Data NOT satisfying condition
- Expected: Processing continues normally

**Test Steps:**
1. Execute Scenario A with TRUE condition
2. Verify expected action occurs
3. Execute Scenario B with FALSE condition
4. Verify action is bypassed
...
"""
        ...
        select_ops = [op for op in self.db_operations if op.operation_type == 'SELECT'][:2]
        for op in select_ops:
            doc += f"""### P1-{p1_count:03d}: Database Query - {op.table}

**Objective:** Verify data retrieval from `{op.table}` table

**Test Scenarios:**

**Scenario A: Data Exists**
- Input: Valid query parameters
- Expected: Returns matching records

**Scenario B: No Data Found**
- Input: Query parameters with no matches
- Expected: Returns empty result set

**Test Steps:**
1. Seed database with known test data
2. Execute query with matching criteria
3. Verify correct records returned
4. Execute query with non-matching criteria
5. Verify empty result set

**Expected Results:**
- Query returns correct data when available
...
These tests verify the API handles unusual scenarios and errors gracefully.

"""
        
        # Generate P2 tests from validations
        p2_count = 1
        for validation in self.validations[:5]:
            doc += f"""### P2-{p2_count:03d}: Validation - {validation.rule_type}

**Objective:** Verify input validation for `{validation.field}` field

**Test Data:**

**Invalid Case:**
- Field: `{validation.field}`
- Value: {self._get_invalid_test_value(validation.rule_type)}
- Expected Error: "{validation.message if validation.message else 'Validation error'}"

**Valid Case:**
- Field: `{validation.field}`
- Value: {self._get_valid_test_value(validation.rule_type)}
- Expected: Validation passes

**Test Steps:**
1. Attempt operation with invalid `{validation.field}`
2. Verify validation error is raised
3. Verify error message matches expected
4. Retry with valid `{validation.field}`
...
**Legacy Reference:** Line {validation.line_number}
...
"""
        ...
        # Remaining business rules as edge cases
        for rule in self.business_rules[6:9]:
            doc += f"""### P2-{p2_count:03d}: Edge Case - {rule.name}

**Objective:** Verify system handles edge condition correctly

**Edge Condition:**
- Condition: `{rule.condition}`
- Boundary: Test values at the edge of the condition

**Test Steps:**
1. Identify boundary values for condition
2. Test with value just meeting condition (boundary +)
3. Test with value just failing condition (boundary -)
4. Verify correct behavior in both cases

**Expected Results:**
- Boundary values handled correctly
- No off-by-one errors
- Consistent behavior across boundaries
...
"""
        ...
        if self.db_operations:
            doc += f"""### P2-{p2_count:03d}: Database Transaction Rollback

**Objective:** Verify database transactions rollback correctly on failure
...
**Test Steps:**
1. Begin transaction
2. Perform partial database operations
3. Simulate error (connection loss, constraint violation)
4. Verify rollback occurs
5. Confirm no partial data persisted
...
**Legacy Reference:** Database operations at lines {', '.join(str(op.line_number) for op in self.db_operations[:3])}
...
**Total Test Scenarios:** {p0_count + p1_count + p2_count - 3}
- P0 (Critical): {p0_count - 1} tests
- P1 (Happy Path): {p1_count - 1} tests
- P2 (Edge Cases): {p2_count - 1} tests

**Recommended Execution Order:**
1. Run all P0 tests first - must achieve 100% pass rate
2. Run P1 tests - target 95%+ pass rate
3. Run P2 tests - target 90%+ pass rate
...
- P0 tests: Automate in CI/CD pipeline (blocking)
- P1 tests: Automate in nightly builds
- P2 tests: Automate weekly regression suite

---

**Generated by CORTEX Lens v3.0** - For questions or issues, contact: github.com/asifhussain60/CORTEX
"""
        
        return doc
    
    def _get_invalid_test_value(self, rule_type: str) -> str:
        """Get an example invalid value for a validation rule type."""
        rule_lower = rule_type.lower()
        if 'null' in rule_lower or 'empty' in rule_lower:
            return 'null or empty string'
        elif 'length' in rule_lower:
            return 'string exceeding max length'
        elif 'format' in rule_lower:
            return 'improperly formatted value'
        elif 'range' in rule_lower:
            return 'value outside acceptable range'
        else:
            return 'invalid value'
    
    def _get_valid_test_value(self, rule_type: str) -> str:
        """Get an example valid value for a validation rule type."""
        rule_lower = rule_type.lower()
        if 'null' in rule_lower or 'empty' in rule_lower:
            return 'non-empty string'
        elif 'length' in rule_lower:
            return 'string within length limits'
        elif 'format' in rule_lower:
            return 'properly formatted value'
        elif 'range' in rule_lower:
            return 'value within acceptable range'
        else:
            return 'valid value'
...
"""
    ...
    def _parse_action(self) -> str:
        """Parse the action from class/method names with narrative validation."""
        # Use narrative validator if available
        if self.enable_narrative_validation and self.narrative_validator:
            return self.narrative_validator.humanize_class_name(self.class_name)
...
"""
    ...
    def _generate_diagram_files(self):
        """Generate separate .mmd files in diagrams/ folder with proper Mermaid syntax."""
        diagrams_dir = self.output_dir / 'diagrams'
        diagrams_dir.mkdir(parents=True, exist_ok=True)
        
        # Generate flowchart.mmd (pure Mermaid, no fence markers)
        ...
        # Generate dependency.mmd (conditional, pure Mermaid)
    ...
    # Public diagram generation methods (called by CLI wrapper)
    ...
    def generate_dependency_diagram(self) -> str:
        ...
        return self._generate_dependency_diagram_content() or "graph LR\n    A[No Dependencies Detected]"
    ...
    def generate_all(self):
        ...
        # Generate test scenarios in dedicated tests/ folder
        tests_dir = self.output_dir / 'tests'
        tests_dir.mkdir(parents=True, exist_ok=True)
        print(f"   📋 Generating test scenarios...")
        test_scenarios = self.generate_test_scenarios()
        test_scenarios_path = tests_dir / 'test-scenarios.md'
        test_scenarios_path.write_text(test_scenarios, encoding='utf-8')
        print(f"   ✅ tests/test-scenarios.md ({len(test_scenarios)} chars)")

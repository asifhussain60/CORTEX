...
class BrainInitOrchestrator:
    ...
    def initialize_brain(self) -> Dict[str, Any]:
        ...
        # Set initial schema versions
        from src.tier0.schema_version_tracker import SchemaVersionTracker
        tracker = SchemaVersionTracker(brain_path=str(self.brain_path))
        for tier, version in tracker.LATEST_VERSIONS.items():
            tracker.set_version(tier, version)

...
class PlanExecutionOrchestrator:
    ...
    def _find_duplicate_code(self, files: List[Path]) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        # Create signature: name + params + body length
                        params = [arg.arg for arg in node.args.args]
                        body_lines = len(node.body)
                        signature = f"{node.name}({','.join(params)}):{body_lines}"
        ...
        # Find duplicates (same signature in multiple locations)
    ...
    def _organize_files(self, files: List[Path]) -> Dict[str, Any]:
        ...
        # Load file structure rules
        rules = {
            'test_': 'tests/',
            'orchestrator': 'src/orchestrators/',
            'collector': 'src/dashboard/collectors/',
            'agent': 'src/cortex_agents/',
            'validator': 'src/operations/validators/',
            'utility': 'src/operations/utilities/'
        }
        ...
        for file in files:
            ...
            if target_dir and not str(file.parent).endswith(str(target_dir).replace('/', '')):
                target_path = target_dir / filename
                target_path.parent.mkdir(parents=True, exist_ok=True)
                
                try:
                    shutil.move(str(file), str(target_path))
                    moves.append({'from': str(file), 'to': str(target_path)})
                    files_moved += 1
                    logger.info(f"Moved {file.name} to {target_dir}")
                except Exception as e:
    ...
    def _update_references(self, files: List[Path]) -> Dict[str, Any]:
        ...
        for file in files:
            ...
            # Extract module path
            old_import = str(file.relative_to(project_root)).replace('/', '.').replace('\\', '.').replace('.py', '')
            ...
            for py_file in all_py_files:
                ...
                try:
                    ...
                    import_pattern = rf'(from\s+{re.escape(old_import)}\s+import|import\s+{re.escape(old_import)})'
                    if re.search(import_pattern, content):
                        updates.append({
                            'file': str(py_file),
                            'old_import': old_import,
                            'note': 'Import reference found - manual review recommended'
                        })
                        updated += 1
                        
                except Exception as e:
                    logger.warning(f"Failed to check imports in {py_file}: {e}")
        
        logger.info(f"Found {updated} files with import references")
        return {
            "success": True,
            "updated": updated,
            "updates": updates
    ...
    def _verify_feature_wiring(self, plan_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check 1: Routes in cortex-operations.yaml
        ops_file = Path('cortex-operations.yaml')
        if ops_file.exists():
            try:
                with open(ops_file, 'r', encoding='utf-8') as f:
                    ops_config = yaml.safe_load(f)
                    if 'operations' in ops_config:
                        checks['routes_configured'] = True
            except Exception as e:
                issues.append(f"Failed to load cortex-operations.yaml: {e}")
        else:
            issues.append("cortex-operations.yaml not found")
        
        # Check 2: Entry points in src/entry_point/
        entry_point_dir = Path('src/entry_point')
        if entry_point_dir.exists():
            py_files = list(entry_point_dir.glob('*.py'))
            if py_files:
                checks['entry_points_found'] = True
            else:
                issues.append("No entry point files found")
        else:
            issues.append("Entry point directory not found")
        ...
        if config_file.exists():
            ...
            issues.append("cortex.config.json not found")
        
        # Determine overall status
        all_passed = all(checks.values())
        status = "fully_wired" if all_passed else "partially_wired"
        
        logger.info(f"Feature wiring status: {status}")
        if issues:
            logger.warning(f"Wiring issues: {', '.join(issues)}")
    ...
    def _run_integration_tests(self, plan_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not test_dir.exists():
            logger.warning("Integration test directory not found")
            return {
                "success": False,
                "passed": 0,
                "failed": 0,
                "error": "Integration tests directory not found"
            }
        
        try:
            # Run pytest with JSON output
            result = subprocess.run(
                ['pytest', str(test_dir), '--json-report', '--json-report-file=test_report.json', '-v'],
                capture_output=True,
                text=True,
            ...
            if report_file.exists():
                ...
                logger.info(f"Integration tests (parsed): {passed} passed, {failed} failed")
                return {

...
class DocSyncHook:
    ...
    def install_git_hook(self) -> bool:
        """
        Install post-commit git hook for automatic doc sync detection.
        
        Hook script:
        - Detects changes to agents/orchestrators/templates
        - Logs detection (doesn't auto-sync, respects deploy-only policy)
        - Provides reminder message to run deploy for doc sync
        ...
        if not hooks_dir.exists():
            print("⚠️  No .git/hooks directory found - not a git repository")
            return False
...
\"\"\"
CORTEX Post-Commit Hook - Doc Sync Detection

Detects changes that may require documentation updates.
Does NOT auto-sync (respects deploy-only policy).
\"\"\"
...
try:
    ...
    if changes['requires_doc_update']:
        categories = ', '.join(changes['categories'])
        print(f"\\n📝 Doc sync recommended - changes detected in: {categories}")
        print("   Run 'python scripts/deploy_cortex_simple.py' to synchronize documentation\\n")

except Exception as e:
    # Silent fail - don't block commits on hook errors
    pass
"""
        ...
        print(f"✅ Git post-commit hook installed: {hook_file}")
        return True
    
    def detect_doc_sync_needed(self) -> bool:
        """
        Simple boolean check if doc sync is needed.
        
        Returns:
            True if documentation update recommended
        """
        changes = self.detect_changes()
        return changes.get('requires_doc_update', False)
    
    def sync_documentation(self) -> Dict[str, Any]:
        """
        Perform full documentation synchronization.
        
        Alias for update_docs() for test compatibility.
        
        Returns:
            Result dict with success/failure details
        """
        changes = self.detect_changes()
        return self.update_docs(changes)
    
    def validate_generated_docs(self) -> bool:
        """
        ...
        """
        validation_result = self._validate_docs()

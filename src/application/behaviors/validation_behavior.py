...
class ValidationBehavior(IPipelineBehavior):
    ...
    async def handle(
        ...
        try:
            ...
            validation_errors.extend(self._validate_common_fields(request))
            
            # Validate command-specific fields
            if self._is_capture_conversation_command(request):
                validation_errors.extend(self._validate_capture_conversation(request))
            elif self._is_learn_pattern_command(request):

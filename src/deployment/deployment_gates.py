...
class DeploymentGates:
    ...
    def _validate_template_format(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load and validate new template architecture (v4.0+)
            ...
            if not schema_version.startswith("4."):
                warnings.append(f"Schema version {schema_version} (expected 4.0+)")
    ...
    def _validate_cortex_brain_operational(self) -> Dict[str, Any]:
        ...
        if templates_file and templates_file.exists():
            try:
                ...
                # Verify critical sections exist (v4.0 architecture uses routing + sections)
                if 'routing' in data or 'sections' in data:
                    checks["response_templates"] = True
                else:
                    issues.append("response-templates-v4.yaml missing 'routing' or 'sections' key")
            except Exception as e:

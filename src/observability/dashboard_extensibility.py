...
Usage:
    from src.observability.dashboard_extensibility import enrich_dashboard_context
...
# Default handling: None if not set
...
def validate_endpoint_type(value: str) -> bool:
    """
    Type validation for endpoint configuration.
    
    Args:
        value: Endpoint value to validate.
    
    Returns:
        True if valid, False otherwise.
    """
    if value is None:
        return True  # None is valid (disabled)
...
# Fallback strategy for testing
def test_fallback_strategy() -> bool:
    """
    Test that fallback strategy works correctly.
    
    Returns:
        True if fallback works correctly.
    """
    # Save original state
    original_enabled = DOMAIN_EXTENSION_ENABLED
    
    try:
        # Test with disabled domain
        test_data = {"metric": "test", "value": 1}
        result = enrich_dashboard_context(test_data)
        
        # Should return original data unchanged
        return result == test_data
    finally:
        pass  # State is restored automatically

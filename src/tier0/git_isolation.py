...
class GitIsolationEnforcer:
    ...
    def install_hooks(self) -> bool:
        ...
        if pre_commit_installed and pre_push_installed:
            logger.info(f"✅ Git isolation hooks installed in {self.user_repo_path}")
            return True
        else:
            logger.warning("⚠️ Some hooks failed to install")
            return False
...
if [ -n "$VIOLATIONS" ]; then
    echo ""
    echo "========================================================"
    echo "BLOCKED: CORTEX Git Isolation Violation"
    echo "========================================================"
    echo ""
    echo "The following CORTEX files are staged for commit:"
    echo "$VIOLATIONS"
    echo ""
    echo "[X] CORTEX source code MUST NOT be committed to user repos!"
    echo ""
    echo "[OK] CORRECT SETUP:"
    ...
    echo ""
    echo "Documentation: cortex-brain/brain-protection-rules.yaml"
    echo "Rule: GIT_ISOLATION_ENFORCEMENT (Tier 0 Instinct)"
    echo ""
    exit 1
fi

echo "[OK] No CORTEX code detected. Commit allowed."
exit 0
"""
        return script
...
if [ -n "$VIOLATIONS" ]; then
    echo ""
    echo "========================================================"
    echo "BLOCKED: CORTEX Code About to Be Pushed!"
    echo "========================================================"
    echo ""
    echo "CORTEX files detected in push:"
    echo "$VIOLATIONS"
    echo ""
    echo "[X] This would expose CORTEX source code to user repository!"
    echo ""
    echo "FIX OPTIONS:"
...
echo "[OK] No CORTEX code in push. Safe to proceed."
exit 0
"""
        return script

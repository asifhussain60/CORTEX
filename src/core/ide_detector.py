...
1. Environment variables (VSCODE_*, VS_*, CORTEX_IDE)
2. Parent process inspection (Code.exe, devenv.exe)
3. Directory markers (.vscode/, .vs/)
4. Cached context (cortex-brain/ide-context.json)
...
try:
    ...
    logging.warning("psutil not available - parent process detection disabled")
...
class IDEDetector:
    ...
    Usage:
        # Detect IDE
        ide_type = IDEDetector.detect(workspace_root)
        
        # Get config filename
        config_file = IDEDetector.get_config_filename(ide_type)
        
        # Get IDE directory
        ide_dir = IDEDetector.get_ide_directory(ide_type)
    """
    ...
    def detect(cls, workspace_root: Path) -> IDEType:
        ...
        if explicit_ide:
            try:
                ...
                logger.debug(f"IDE detected via CORTEX_IDE: {cls._cached_ide.value}")
                return cls._cached_ide
            except ValueError:
                logger.warning(f"Invalid CORTEX_IDE value: {explicit_ide}")
        ...
        if os.getenv("VSCODE_PID") or os.getenv("VSCODE_IPC_HOOK"):
            ...
            cls._save_context(workspace_root, cls._cached_ide)
            logger.debug("IDE detected via VSCODE_* env vars: vscode")
            return cls._cached_ide
        
        if os.getenv("VisualStudioVersion") or os.getenv("VSINSTALLDIR"):
            cls._cached_ide = IDEType.VISUAL_STUDIO
            cls._save_context(workspace_root, cls._cached_ide)
            logger.debug("IDE detected via VS_* env vars: visualstudio")
            return cls._cached_ide
        ...
        if PSUTIL_AVAILABLE:
            try:
                ...
                if "code" in parent_name:  # Code.exe, code.exe, VSCode
                    cls._cached_ide = IDEType.VSCODE
                    cls._save_context(workspace_root, cls._cached_ide)
                    logger.debug(f"IDE detected via parent process: vscode ({parent_name})")
                    return cls._cached_ide
                
                if "devenv" in parent_name or "visualstudio" in parent_name:
                    cls._cached_ide = IDEType.VISUAL_STUDIO
                    cls._save_context(workspace_root, cls._cached_ide)
                    logger.debug(f"IDE detected via parent process: visualstudio ({parent_name})")
                    return cls._cached_ide
        ...
        if vscode_time > vs_time and vscode_time > 0:
            cls._cached_ide = IDEType.VSCODE
            cls._save_context(workspace_root, cls._cached_ide)
            logger.debug("IDE detected via directory markers: vscode (.vscode/)")
            return cls._cached_ide
        elif vs_time > 0:
            cls._cached_ide = IDEType.VISUAL_STUDIO
            cls._save_context(workspace_root, cls._cached_ide)
            logger.debug("IDE detected via directory markers: visualstudio (.vs/)")
            return cls._cached_ide
        ...
        if cached:
            ...
            logger.debug(f"IDE detected via cached context: {cls._cached_ide.value}")
            return cls._cached_ide
        ...
        logger.info(f"Could not detect IDE, using shared configuration (detection took {elapsed:.2f}ms)")
        return cls._cached_ide
    ...
    def _save_context(cls, workspace_root: Path, ide_type: IDEType) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).debug(f"Failed to save IDE context: {e}")
    ...
    def _load_context(cls, workspace_root: Path) -> Optional[IDEType]:
        ...
        try:
            ...
            logging.getLogger(__name__).debug(f"Failed to load IDE context: {e}")
            return None

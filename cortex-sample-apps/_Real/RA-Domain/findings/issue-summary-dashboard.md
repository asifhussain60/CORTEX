...
The Payment Accounts codebase has **5 critical technical debt items** with a combined annual financial impact of **$645,000** and estimated remediation effort of **161 hours** (4 weeks FTE).
...
#### TD-002: Zero Test Coverage for Rollover Logic
**Type:** Test Gap  
**Severity:** CRITICAL  
...
**Description:**
The core year-end rollover processing logic has **0% test coverage**, creating massive regulatory and financial risk. The service handles RegulatoryAgency-compliant rollover calculations ($640 FlexAccount limit, 100% HealthSavings rollover, etc.) and processes billions of dollars annually.

**Business Impact:**
- **Regulatory Risk:** RegulatoryAgency audit failures could trigger penalties
...
- **Annual Cost:** $500,000 (incident response, manual reconciliation, customer support)

**Remediation:**
- **Effort:** 40 hours
- **Approach:** TDD for critical paths (FlexAccount/HealthSavings/HealthReimbursement rollover calculations)
- **Coverage Target:** 80% line coverage, 100% branch coverage for RegulatoryAgency rules
- **ROI:** $12,500 per hour ($500k ÷ 40 hrs)

**Test Cases Needed:**
1. FlexAccount rollover calculation (balance ≤ $640)
2. FlexAccount expiration calculation (balance > $640)
3. HealthSavings 100% rollover
4. HealthReimbursement organization-defined rollover
5. DependentCare 100% expiration
...
1. Rollover calculations (plan type-specific logic)
...
- **Method Count:** 15+ methods (source: rollover-service-methods.json)
...
**Description:**
Requests processing mixes read operations (balance queries) with write operations (request approval/denial), violating **Command Query Responsibility Segregation (CQRS)**.

**Issues:**
- Complex queries joined with write transactions
- N+1 query risks in request detail fetching
- Poor caching opportunities (reads mixed with writes)
- Scalability bottleneck (cannot scale reads independently)

**Business Impact:**
- **Performance:** Slow request approval workflows
- **Scalability:** Cannot handle 10x request volume
...
**Type:** Code Complexity  
**Severity:** HIGH  
**File:** `PaymentAccountBalanceService.cs` (inferred)
...
**Business Impact:**
- **Performance:** Complex calculations slow down customer portal
...
- **Approach:** Split into:
  ...
  - `PendingBalanceService` - Requests in-flight
...
- Test coverage: 80% line, 100% branch for RegulatoryAgency rules

**Success Criteria:**
- All rollover calculations have passing tests
...
**Focus:** CQRS pattern  
**Items:**
- ✅ **TD-003** - Implement CQRS for Requests (35 hours, $40k savings)

**Deliverables:**
- ClaimQueryHandler (read model)
- ClaimCommandHandler (write model)
- Event sourcing for request state changes (optional)
...
### RegulatoryAgency Compliance (TD-002)
**Risk:** Incorrect rollover calculations violate RegulatoryAgency regulations  
...
- `rollover-service-methods.json` - 717 LOC service analysis

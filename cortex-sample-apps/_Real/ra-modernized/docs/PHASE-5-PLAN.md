...
**Project:** RA Funding Invoices Migration (Product.RA.Api)  
...
1. **Updater_CreateRAFundingInvoices** → `FundingInvoiceBatchService`
2. **XGenerateFundingInvoice** → `FundingInvoiceTransactionService`
...
### Task 5.1: Migrate Updater_CreateRAFundingInvoices
...
**Current State:**
- `IFundingInvoiceBatchService` exists with `CreateBatchAsync()` method
- Method creates a funding batch but doesn't process individual invoices

**Target State:**
- New `CreateBatchInvoicesAsync(CreateBatchInvoicesDto dto)` method
- Processes list of subaccount IDs
- Creates individual invoices for each subaccount
...
**WCF Contract (Legacy):**
```xml
<operation name="Updater_CreateRAFundingInvoices">
  <input>
    <EmployerId>12345</EmployerId>
    <SubaccountIds>
      <SubaccountId>1001</SubaccountId>
      <SubaccountId>1002</SubaccountId>
      <!-- ... up to 1000 subaccounts -->
    </SubaccountIds>
    <EffectiveDate>2025-12-15</EffectiveDate>
    <Description>Q4 2025 Funding</Description>
  </input>
  <output>
...
```http
POST /api/fundinginvoices/batch
Content-Type: application/json

{
  "employerId": "12345",
  "subaccountIds": [1001, 1002],
...
2. **Update IFundingInvoiceBatchService Interface**
   ```csharp
   Task<BatchInvoiceResultDto> CreateBatchInvoicesAsync(CreateBatchInvoicesDto dto);
   ```

3. **Implement in FundingInvoiceBatchService**
   - Validate employer exists
   - Create funding batch
   - For each subaccount ID:
     - Get subaccount details
     - Calculate peg amount (invoice amount - balance)
     - If peg > 0, create funding invoice
     - Track success/failure
   - Return batch result with counts

4. **Business Logic Requirements**
   - Skip subaccounts that already have invoices for the effective date (duplicate prevention)
...
5. **Error Handling**
   ...
   - Empty subaccount list → 400 Bad Request
   - Employer not found → 404 Not Found
   - Individual subaccount failures → Include in `failedSubaccounts` list

**Test Scenarios:**
- Happy path: 10 subaccounts, all succeed
- Partial success: 10 subaccounts, 8 succeed, 2 fail (subaccount not found)
- Duplicate prevention: Skip subaccounts with existing invoices
- Empty employer list → 400
- Large batch: 1000 subaccounts (performance test)
...
### Task 5.2: Migrate XGenerateFundingInvoice
...
**Current State:**
- `IFundingInvoiceTransactionService` exists with `CreateInvoiceAsync()` method
- Method creates invoice but doesn't check balance vs peg logic

**Target State:**
- New `GenerateFundingInvoiceAsync(GenerateFundingInvoiceDto dto)` method
- Checks subaccount balance vs peg amount
...
**WCF Contract (Legacy):**
```xml
<operation name="XGenerateFundingInvoice">
  <input>
    <SubaccountId>1001</SubaccountId>
...
**REST Equivalent (Target):**
```http
POST /api/fundinginvoices/generate
Content-Type: application/json
...
1. **Create DTO Classes**
   - `GenerateFundingInvoiceDto` (request)
   - `GenerateFundingInvoiceResultDto` (response)

2. **Update IFundingInvoiceTransactionService Interface**
   ```csharp
   Task<GenerateFundingInvoiceResultDto> GenerateFundingInvoiceAsync(GenerateFundingInvoiceDto dto);
   ```

3. **Implement in FundingInvoiceTransactionService**
   - Get subaccount details (balance, employer ID)
   - Calculate peg amount: `InvoiceAmount - SubaccountBalance`
   - If peg > 0:
...
4. **Business Logic Requirements**
   ...
   - Auto-debit logic:
     - If subaccount.AutoDebitEnabled = true
...
5. **Error Handling**
   - Subaccount not found → 404 Not Found
...
1. **Unit Tests (Service Layer)**
   - FundingInvoiceBatchService (new methods)
   - FundingInvoiceTransactionService (new methods)
...
3. **Edge Case Tests**
   ...
   - Large data volumes (1000 subaccounts)
   - Concurrent requests
   - Timeout scenarios
...
5. **Performance Tests**
   - Batch processing: 100 subaccounts <5 seconds
   - Single invoice generation: <500ms
   - Database query optimization

**Test File Organization:**
```
tests/RA.FundingInvoices.Core.Tests/
├── Services/
│   ├── FundingInvoiceBatchServiceTests.cs
│   ├── FundingInvoiceTransactionServiceTests.cs
│   └── Legacy/
│       ├── CreateBatchInvoicesTests.cs (Task 5.1 tests)
│       └── GenerateFundingInvoiceTests.cs (Task 5.2 tests)
└── Integration/
...
1. **Complete Batch Workflow**
   - POST /api/fundinginvoices/batch → Create batch with 10 invoices
   - GET /api/fundingbatches/{batchId} → Retrieve batch
   - POST /api/fundingbatches/{batchId}/close → Close batch
   - Verify database state (batch status, invoice statuses, totals)

2. **Generate Invoice Workflow**
   - POST /api/fundinginvoices/generate → Create invoice (balance below peg)
   - GET /api/fundinginvoices/{invoiceId} → Retrieve invoice
...
public async Task CompleteBatchWorkflow_Success()
{
    ...
    // Act 1: Create batch
    var createResponse = await client.PostAsJsonAsync("/api/fundinginvoices/batch", dto);
    var batchResult = await createResponse.Content.ReadFromJsonAsync<BatchInvoiceResultDto>();
    
    // Act 2: Close batch
    var closeResponse = await client.PostAsync($"/api/fundingbatches/{batchResult.BatchId}/close", null);
    
    // Assert: Verify state
    var batch = await _dbContext.FundingBatches.FindAsync(batchResult.BatchId);
...
2. **Create Shadow Test Scenarios**
   ...
   - Include high-volume scenarios (batch with 500+ subaccounts)
...
1. **Phase 1: Automated Test Suite (1000+ scenarios)**
   ...
   - Run 500 production-like scenarios (real employer/subaccount data)
...
| GenerateFundingInvoiceAsync | Transaction service migration | ~120 LOC |
...
**Success Criteria:**
- ✅ Both service methods implemented (CreateBatchInvoicesAsync, GenerateFundingInvoiceAsync)
...
dotnet run --project tests/RA.FundingInvoices.ContractTests -- --dry-run
```
...
dotnet run --project tests/RA.FundingInvoices.ContractTests

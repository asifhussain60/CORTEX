...
**Purpose:** HIPAA/SOC2 compliant field-level encryption using Azure Key Vault  
**Version:** 1.0  
...
This guide explains the field-level encryption infrastructure for protecting PHI (Protected Health Information) in the RA Funding Invoices API.

**Compliance Requirements:**
- HIPAA: Encrypt PHI at rest (SSN, account numbers, member names, DOB)
- SOC2: Cryptographic key management with access controls
...
az group create --name rg-ra-funding --location eastus
...
```json
{
  ...
  "AzureKeyVault": {
    "Url": "https://kv-ra-funding-prod.vault.azure.net/",
...
**POST /api/fundinginvoices** (Create invoice)
```json
{
  "memberName": "John Doe",        // Automatically encrypted before DB insert
  "memberSSN": "123-45-6789",      // Automatically encrypted
...
```
MemberName: "AQAAAAEAACcQ...encrypted-base64...==  "
MemberSSN: "BgAAAAIAADdR...encrypted-base64...=="
```

**GET /api/fundinginvoices/123** (Retrieve invoice)
```json
{
  "id": 123,
  "memberName": "John Doe",        // Automatically decrypted before response
...
public class FundingInvoicesController : ControllerBase
{
    ...
    public FundingInvoicesController(IEncryptionService encryptionService)
    {
...
public async Task ReencryptAllRecords()
{
    ...
    {
        if (!string.IsNullOrEmpty(invoice.MemberSSN))
        {
            // Decrypt with old key version
            var plaintext = await _encryptionService.DecryptAsync(invoice.MemberSSN);
            
            // Re-encrypt with new key version
            invoice.MemberSSN = await _encryptionService.EncryptAsync(plaintext);
        }
    }
...
- Selective encryption: Only PHI fields, not all data
...
✅ **DO:**
- Encrypt only PHI/PII fields (SSN, names, account numbers)
...
az keyvault show --name kv-ra-funding-prod

...
**Audience:** New developers joining the RA Funding Invoices migration project  
...
🎉 **You're running!** Try creating a funding invoice via the interactive UI.
...
ra-modernized/
├── src/
│   ├── RA.FundingInvoices.API/              # REST API (Controllers + Middleware)
│   ├── RA.FundingInvoices.Core/             # Business logic (Services + DTOs)
│   └── RA.FundingInvoices.Infrastructure/   # Data access (Repositories)
└── tests/
    ├── RA.FundingInvoices.UnitTests/        # Fast isolated tests
    ├── RA.FundingInvoices.IntegrationTests/ # Database + API tests
    ├── RA.FundingInvoices.ContractTests/    # Schema validation
    └── RA.FundingInvoices.API.Tests/        # HTTP endpoint tests
```
...
curl -X POST https://localhost:7001/api/v1/funding-invoices \
...
- See [03-EFCORE-MIGRATION.md](./03-EFCORE-MIGRATION.md) for migration guide
...
**File:** `tests/RA.FundingInvoices.UnitTests/Services/FundingInvoiceServiceTests.cs`
...
public async Task CreateAsync_WithZeroAmount_ShouldThrowValidationException()
{
    ...
    var request = new CreateFundingInvoiceRequest
    {
        ...
        SubaccountId = "SA-001",
        ReimbursementPlanId = "RP-001",
        EmployerFundingDefault = 0,  // Zero amount
        EmployeeFundingDefault = 0,  // Zero amount
...
**Run the test:**
```bash
dotnet test tests/RA.FundingInvoices.UnitTests/ --filter "CreateAsync_WithZeroAmount"
```
...
2. **Code Review:** `C:\PROJECTS\Platform.Classic\cortex\ra-modernized\.review\MIGRATION_ANALYSIS_REPORT.md`
...
5. **EF Core Migration:** [03-EFCORE-MIGRATION.md](./03-EFCORE-MIGRATION.md)
...
dotnet watch test --project tests/RA.FundingInvoices.UnitTests/
...
# Verify API works
dotnet run --project src/RA.FundingInvoices.API
# Test via Swagger UI
```
...
- [x] Access Swagger UI
- [x] Create a funding invoice via API

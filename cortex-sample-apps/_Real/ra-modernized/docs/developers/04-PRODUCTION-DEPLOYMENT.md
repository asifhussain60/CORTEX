...
**Objective:** Get representative sample without exposing PII/PHI
...
SELECT
    ...
    'SA-' + CAST(ROW_NUMBER() OVER (ORDER BY SubaccountId) AS NVARCHAR) AS SubaccountId,
    Amount,
    Status,
    InvoiceType,
    EffectiveDate,
...
WHERE CreatedDate >= DATEADD(MONTH, -3, GETDATE())  -- Last 3 months
    AND EmployerId IN (SELECT TOP 10 EmployerId FROM FundingInvoices GROUP BY EmployerId ORDER BY COUNT(*) DESC);
...
```bash
dotnet test tests/RA.FundingInvoices.ProductionValidation/
```
...
|----------|--------------|---------------|------------|
| **Employer contribution split** | `EmployerFundingDefault` | Mapped to `InvoiceLineItems` | Sum equals invoice amount |
| **Employee contribution split** | `EmployeeFundingDefault` | Mapped to `InvoiceLineItems` | Sum equals invoice amount |
...
| **Peg amount calculation** | `GenerateFundingInvoice` logic | Service layer calculation | Amounts match |
...
public async Task LegacyEmployerSplit_ShouldMatchModernLineItems()
{
    ...
    {
        EmployerFundingDefault = legacyInvoice.EmployerFundingDefault,
        EmployeeFundingDefault = legacyInvoice.EmployeeFundingDefault,
        // ... other fields
    };
...
public async Task CreateInvoice_NullOptionalFields_ShouldSucceed()
{
    var request = new CreateFundingInvoiceRequest
    {
        ...
        EmployerFundingDefault = 500,
        EmployeeFundingDefault = 250,
...
**File:** `tests/LoadTests/funding-invoice-load-test.js`
...
export default function () {
    ...
    let createResponse = http.post(
        'https://your-api.com/api/v1/funding-invoices',
        payload,
        { headers: headers }
    );
    ...
    if (createResponse.status === 201) {
        ...
        let getResponse = http.get(
            `https://your-api.com/api/v1/funding-invoices/${invoice.invoiceId}`,
            { headers: headers }
        );
...
```bash
k6 run tests/LoadTests/funding-invoice-load-test.js
```
...
using (var operation = _telemetryClient.StartOperation<RequestTelemetry>("CreateFundingInvoice"))
{
...
alert:
  name: "Funding Invoices - High Error Rate"
...
alert:
  name: "Funding Invoices - Slow Response"
...
# Alert: High CPU
alert:
  name: "Funding Invoices API - High CPU"
...
**File:** `docs/operations/RUNBOOK-HIGH-ERROR-RATE.md`

```markdown
# Runbook: High Error Rate (>1%)

## Alert Details
- **Trigger:** Funding Invoice API error rate exceeds 1% for 5 minutes
- **Severity:** Critical
- **Impact:** Users unable to create/update funding invoices
...
```markdown
# Operations Team Training - Funding Invoices API
...
**Deliverable:** Signed training completion form from all ops team members
...
**Previous Step:** [← EF Core Migration Guide](./03-EFCORE-MIGRATION.md)  

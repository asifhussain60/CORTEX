...
# API tests
dotnet test tests/RA.FundingInvoices.API.Tests/
```
...
```
tests/
├── RA.FundingInvoices.UnitTests/          # 87 tests, 3,142 LOC
│   ├── Services/
│   │   ├── FundingInvoiceServiceTests.cs   (45 tests)
│   │   └── FundingBatchServiceTests.cs     (42 tests)
│   ├── Validators/
│   │   └── CreateFundingInvoiceValidatorTests.cs
│   └── Utilities/
│       └── TestHelpers.cs
│
├── RA.FundingInvoices.IntegrationTests/   # 42 tests, 1,821 LOC
│   ├── Repositories/
│   │   ├── EFCoreFundingInvoiceRepositoryTests.cs
...
│       └── DatabaseFixture.cs
│
├── RA.FundingInvoices.ContractTests/      # 15 tests, 1,144 LOC
│   ├── Schemas/
│   │   └── FundingInvoiceSchemaTests.cs
│   └── Validators/
│       └── ApiContractValidator.cs
│
└── RA.FundingInvoices.API.Tests/          # 28 tests, 1,464 LOC
    ├── Controllers/
    │   ├── FundingInvoiceControllerTests.cs
    │   └── FundingBatchControllerTests.cs
    └── Middleware/
...
public class FundingInvoiceServiceTests
{
    ...
    private readonly Mock<IValidator<CreateFundingInvoiceRequest>> _mockValidator;
    private readonly Mock<IReimbursementPlanAdapter> _mockPlanAdapter;
    private readonly FundingInvoiceService _service;
    
    public FundingInvoiceServiceTests()
    {
        _mockRepo = new Mock<IFundingInvoiceRepository>();
        _mockValidator = new Mock<IValidator<CreateFundingInvoiceRequest>>();
        _mockPlanAdapter = new Mock<IReimbursementPlanAdapter>();
    ...
    public async Task CreateAsync_ValidRequest_ShouldCreateInvoice()
    {
        ...
        var request = new CreateFundingInvoiceRequest
        {
            ...
            EmployerFundingDefault = 500.00m,
            EmployeeFundingDefault = 250.00m,
        ...
        _mockPlanAdapter.Setup(a => a.GetPlanAsync(request.ReimbursementPlanId))
            .ReturnsAsync(new ReimbursementPlan { PlanId = "RP-001", IsActive = true });
        ...
        _mockRepo.Verify(r => r.CreateAsync(It.Is<FundingInvoice>(
    ...
    public async Task CreateAsync_InvalidRequest_ShouldThrowValidationException()
    {
        ...
        var request = new CreateFundingInvoiceRequest
        {
            ...
            EmployerFundingDefault = -100  // Invalid: negative
        };
        
        var validationErrors = new List<ValidationFailure>
        {
            new ValidationFailure("EmployerId", "EmployerId is required"),
            new ValidationFailure("EmployerFundingDefault", "Amount must be >= 0")
        };
        ...
        _mockRepo.Verify(r => r.CreateAsync(It.IsAny<FundingInvoice>()), Times.Never);
    }
        ...
        decimal employeeAmount)
    {
        ...
        var request = new CreateFundingInvoiceRequest
        {
            ...
            SubaccountId = "SA-001",
            ReimbursementPlanId = "RP-001",
            EmployerFundingDefault = employerAmount,
            EmployeeFundingDefault = employeeAmount,
            EffectiveDate = DateTime.UtcNow,
            CreatedBy = "test"
        };
...
```csharp
public class EFCoreFundingInvoiceRepositoryTests : IClassFixture<DatabaseFixture>
{
    private readonly FundingInvoicesDbContext _context;
    private readonly IFundingInvoiceRepository _repository;
    
    public EFCoreFundingInvoiceRepositoryTests(DatabaseFixture fixture)
    {
    ...
    public async Task CreateAsync_ShouldPersistToDatabase()
    {
        ...
        var savedInvoice = await _context.FundingInvoices
    ...
    public async Task GetByEmployerIdAsync_ShouldReturnOnlyNonVoidedInvoices()
    {
        ...
        var activeInvoice = new FundingInvoice
        {
            InvoiceId = Guid.NewGuid(),
            EmployerId = "EMP-001",
            IsVoided = false,
        ...
        var voidedInvoice = new FundingInvoice
        {
            InvoiceId = Guid.NewGuid(),
            EmployerId = "EMP-001",
...
public class DatabaseFixture : IDisposable
{
    ...
    {
        // Use test database
        _connectionString = "Server=(localdb)\\MSSQLLocalDB;Database=FundingInvoices_Test;Trusted_Connection=True";
    ...
    public FundingInvoicesDbContext CreateContext()
    {
        var options = new DbContextOptionsBuilder<FundingInvoicesDbContext>()
            .UseSqlServer(_connectionString)
            .Options;
        
        var context = new FundingInvoicesDbContext(options);
...
```csharp
public class FundingInvoiceSchemaTests
{
    private readonly JsonSchemaValidator _validator;
    
    public FundingInvoiceSchemaTests()
    {
        // Load OpenAPI schema
        var schemaJson = File.ReadAllText("Schemas/funding-invoice-schema.json");
        _validator = new JsonSchemaValidator(schemaJson);
    }
    
    [Fact]
    public void CreateFundingInvoiceRequest_ValidRequest_ShouldMatchSchema()
    {
        // Arrange
        var requestJson = @"{
            ""employerId"": ""EMP-001"",
    ...
    [Fact]
    public void CreateFundingInvoiceRequest_MissingRequiredFields_ShouldFailValidation()
    {
        // Arrange
        var requestJson = @"{
            ""employerId"": ""EMP-001""
        ...
        validationResult.Errors.Should().Contain(e => 
            e.Path == "subaccountId" && e.Message.Contains("required"));
    }
    ...
    public void CreateFundingInvoiceRequest_InvalidEmployerId_ShouldFailValidation(
        string fieldName, 
        string value)
    {
...
public class FundingInvoiceControllerTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public FundingInvoiceControllerTests(WebApplicationFactory<Program> factory)
    {
    ...
    public async Task CreateInvoice_ValidRequest_ShouldReturn201Created()
    {
        ...
        // Act
        var response = await _client.PostAsync("/api/v1/funding-invoices", content);
        
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Created);
        
        var responseBody = await response.Content.ReadAsStringAsync();
        var invoice = JsonSerializer.Deserialize<FundingInvoiceResponse>(responseBody);
        ...
        response.Headers.Location!.ToString().Should().Contain($"/api/v1/funding-invoices/{invoice.InvoiceId}");
    }
    ...
    public async Task GetInvoice_Unauthorized_ShouldReturn401()
    {
        ...
        // Act
        var response = await client.GetAsync("/api/v1/funding-invoices/some-guid");
...
```csharp
public class FundingInvoiceBuilder
{
    private FundingInvoice _invoice = new()
    {
        InvoiceId = Guid.NewGuid(),
        InvoiceNumber = "INV-TEST-001",
        EmployerId = "EMP-001",
    ...
    public FundingInvoiceBuilder WithEmployerId(string employerId)
    {
        _invoice.EmployerId = employerId;
        return this;
    }
    
    public FundingInvoiceBuilder WithAmount(decimal amount)
    {
        _invoice.Amount = amount;
        return this;
    }
    
    public FundingInvoiceBuilder WithStatus(string status)
    {
        _invoice.Status = status;
        return this;
    }
    
    public FundingInvoice Build() => _invoice;
}

// Usage in tests
var invoice = new FundingInvoiceBuilder()
...
```csharp
public class FundingInvoiceTestData : IEnumerable<object[]>
{
...
// Usage in tests
[Theory]
[ClassData(typeof(FundingInvoiceTestData))]
...
_mockRepo.Setup(r => r.GetByIdAsync(It.IsAny<Guid>()))
    .ReturnsAsync((Guid id) => new FundingInvoice { InvoiceId = id });
...
var mocks = new FundingInvoiceServiceMocksBuilder()
    .WithValidRepository()
    .WithValidValidator()
    .WithActivePlan()
    .Build();
...
```csharp
// ❌ Bad: Magic numbers
var invoice = new FundingInvoice { Amount = 123.45m };

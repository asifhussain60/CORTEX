...
- 5 WCF transaction definitions:
  - `XAddFundingInvoice` → POST /api/fundinginvoices
  - `XGenerateFundingInvoice` → POST /api/fundinginvoices/generate
  - `Updater_CreateRAFundingInvoices` → POST /api/fundinginvoices/batch
  - `XCloseFundingBatch` → POST /api/fundingbatches/{batchId}/close
  - `XUpdateFundingBatch` → PUT /api/fundingbatches/{batchId}
...
- **Happy Paths (3):**
  - Employer + employee funding invoice creation
  - Employer-only funding (no employee contribution)
...
- **Happy Paths (3):**
  - 10 subaccounts successful batch creation
...
- **Targets:**
  ...
  - Batch 100 subaccounts: REST P95 <5000ms
...
**File:** `RA.FundingInvoices.ContractTests.csproj`
...
**Project References:**
- `RA.FundingInvoices.API` (REST API project)
- `RA.FundingInvoices.Core` (domain models, repositories, services)
...
**Execution Commands:**
```bash
cd tests/RA.FundingInvoices.ContractTests
...
```
RA.FundingInvoices.ContractTests/
├── Engine/
...
└── RA.FundingInvoices.ContractTests.csproj   (test project config)
```
...
|-----------|------------|-------------|-----------|
| Single Invoice (XAddFundingInvoice) | Baseline | P95 <500ms | ±20% |
| Batch 100 Subaccounts (Updater_CreateRAFundingInvoices) | Baseline | P95 <5000ms | ±20% |
| Batch Closure (XCloseFundingBatch) | Baseline | P95 <1000ms | ±20% |
...
{
    Task<WcfResponse> XAddFundingInvoiceAsync(AddFundingInvoiceRequest request);
    Task<WcfResponse> XGenerateFundingInvoiceAsync(GenerateFundingInvoiceRequest request);
    Task<WcfResponse> Updater_CreateRAFundingInvoicesAsync(CreateRAFundingInvoicesRequest request);
    Task<WcfResponse> XCloseFundingBatchAsync(CloseFundingBatchRequest request);
    Task<WcfResponse> XUpdateFundingBatchAsync(UpdateFundingBatchRequest request);
}
```
...
2. **Seed Test Data**
   - Create test employers, subaccounts, invoices

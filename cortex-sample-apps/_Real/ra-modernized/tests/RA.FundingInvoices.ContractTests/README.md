...
- 5 WCF transaction mappings (XAddFundingInvoice, XGenerateFundingInvoice, etc.)
...
- **XAddFundingInvoice:** 10 scenarios (3 happy path, 5 errors, 2 edge cases)
- **XGenerateFundingInvoice:** 9 scenarios (3 happy path, 3 errors, 3 boundary conditions)
- **Updater_CreateRAFundingInvoices:** 4 scenarios (3 happy path, 1 error)
- **XCloseFundingBatch:** 9 scenarios (3 happy path, 3 errors, 2 state transitions, 1 edge case)
- **XUpdateFundingBatch:** 4 scenarios (2 happy path, 2 errors)
...
dotnet run --project RA.FundingInvoices.ContractTests
...
- Batch processing 100 subaccounts
- Batch closure with auto-debit

## Business Logic Validation

### Critical Calculations
1. **Invoice Total:** `EmployerFunding + EmployeeFunding = TotalAmount`
2. **Peg Amount:** `InvoiceAmount - SubaccountBalance`
...
| Batch 100 Subaccounts | 3000ms | <5000ms | TBD |
...
- task: DotNetCoreCLI@2
  ...
  inputs:
    command: 'test'
    projects: '**/RA.FundingInvoices.ContractTests.csproj'
...
```
tests/RA.FundingInvoices.ContractTests/
├── Engine/

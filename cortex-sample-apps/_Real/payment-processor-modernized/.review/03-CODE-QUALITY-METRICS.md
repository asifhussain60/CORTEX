...
[← Previous: Functionality Analysis](./02-FUNCTIONALITY-ANALYSIS.md) | [Back to Main Report](./MIGPaymentProcessorTION_ANALYSIS_REPORT.md) | [Next: SOLID Principles →](./04-SOLID-PRINCIPLES.md)
...
|------|---------|----------------|----------------|------------------|
| `XAddTransactionInvoice` | 2 | 15 | 15 | ✅ 1 |
| `XGenerateTransactionInvoice` | 1 | 18 | 18 | ✅ 1 |
| `XCloseTransactionBatch` | 9 | 7 | 22 | ✅ 1 |
| `XUpdateTransactionBatch` | 1 | 3 | 3 | ❌ 0 |
| `XReopenTransactionBatch` | 3 | 2 | 2 | ❌ 0 |
...
**Longest Method:** `XGenerateTransactionInvoice.Execute()` - 118 lines
...
**Longest Method:** `TransactionInvoiceService.CreateAsync()` - 42 lines
...
|------|------------------|---------------|----------------|
| `XAddTransactionInvoice` | 8 | PaymentData, PaymentCommon | HIGH (7/10) |
| `XGenerateTransactionInvoice` | 11 | PaymentData, PaymentCommon, Unity, Paragon | VERY HIGH (9/10) |
| `XCloseTransactionBatch` | 11 | PaymentData, PaymentCommon, Unity, Paragon | VERY HIGH (9/10) |
| `XUpdateTransactionBatch` | 2 | PaymentData | LOW (3/10) |
| `XReopenTransactionBatch` | 2 | PaymentData | LOW (3/10) |
| **AVEPaymentProcessorGE** | **6.8** | **Multiple** | **6.2/10 (High)** |

**Legacy Dependency Issues:**
- ❌ Direct dependency on `PaymentData` (data layer coupling)
...
|-------|----------------|----------------|-------------|
| **API (Controllers)** | ITransactionInvoiceService, ITransactionBatchService | LOW (2/10) | 100% |
...
| **AVEPaymentProcessorGE** | **Interface-based** | **2.25/10 (Low)** | **100%** ✅ |
...
```
tests/
├── PaymentProcessor.TransactionInvoices.UnitTests/        (18 files, 3,142 LOC)
│   ├── Services/                        # Business logic tests
│   ├── Repositories/                    # Data access tests
│   ├── Validators/                      # Validation tests
│   ├── Middleware/                      # Middleware tests
│   └── FeatureManagement/              # Feature flag tests
├── PaymentProcessor.TransactionInvoices.IntegrationTests/ (10 files, 1,821 LOC)
│   ├── SchemaValidation/               # Contract tests
│   ├── Monitoring/                     # Metrics tests
│   └── FeatureManagement/             # Integration tests
├── PaymentProcessor.TransactionInvoices.ContractTests/    (3 files, 1,144 LOC)
│   ├── Engine/                         # Verification engine
│   ├── Reporting/                      # Test reports
│   └── Tests/                          # Scenario validation
└── PaymentProcessor.TransactionInvoices.API.Tests/        (4 files, 1,464 LOC)
...
1. **Auto-debit Payment Creation** (72 lines duplicated)
   - `XGenerateTransactionInvoice.cs` lines 104-130
   - `XCloseTransactionBatch.cs` lines 94-120
   - **Impact:** 2 files, ~100% identical logic

2. **Benefit Group Description** (24 lines duplicated)
   - `XGenerateTransactionInvoice.cs` lines 36-42
   - `XCloseTransactionBatch.cs` lines 52-58
   - **Impact:** 2 files, string concatenation logic

3. **AccountCategory/Plan Resolution** (18 lines duplicated)
...
1. **Auto-debit Payment Creation**
   - ✅ Extracted to `IPaymentPlanAdapter.ProcessAutoDebitAsync()`
   - Single implementation shared by all services

2. **Benefit Group Description**
   - ✅ Extracted to `IPaymentPlanAdapter.GetBenefitGroupDescriptionAsync()`
...
|------|-----|----|---------| -------|
| `XAddTransactionInvoice` | 138 | 15 | 48 | ❌ LOW |
| `XGenerateTransactionInvoice` | 141 | 18 | 42 | ❌ LOW |
| `XCloseTransactionBatch` | 267 | 22 | 38 | ❌ LOW |
| `XUpdateTransactionBatch` | 52 | 3 | 72 | ⚠️ MODEPaymentProcessorTE |
| `XReopenTransactionBatch` | 59 | 2 | 75 | ⚠️ MODEPaymentProcessorTE |
| **AVEPaymentProcessorGE** | **131** | **12** | **52** | **❌ LOW** |

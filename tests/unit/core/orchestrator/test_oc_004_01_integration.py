...
class TestComplexRealWorldScenarios:
    ...
    def test_scenario_full_workflow_with_errors(self, event_registry):
        """Scenario: Full workflow with error injection."""
        domain_configs = [
            ("planning", OrchestrationDomain.PLANNING, -1),
            ("design", OrchestrationDomain.DESIGN, 2),  # Error on turn 2
            ("implementation", OrchestrationDomain.IMPLEMENTATION, -1),
        ]

        for domain_name, domain_enum, failure_turn in domain_configs:
            orch = MockDomainOrchestrator(domain_enum, failure_on_turn=failure_turn)
            protocol = ConversationProtocol(
                orchestrator=orch,
                max_turns=10,
                token_limit=10000,
            ...
            successful = 0
            for turn in range(5):
                result = protocol.execute_turn(f"{domain_name} {turn}", {})
                if result.is_ok():
                    successful += 1
    ...
    def test_scenario_token_constrained_workflow(self, event_registry):
        """Scenario: Workflow with tight token constraints."""
        orch = MockDomainOrchestrator(OrchestrationDomain.PLANNING)
        protocol = ConversationProtocol(
            orchestrator=orch,
            max_turns=100,
            token_limit=50,  # Very tight
            event_registry=event_registry,
        )

...
class TestEnvironmentDetector:
    ...
    def test_detect_mcp_environment(self, detector):
        ...
        with patch.dict(os.environ, {'CORTEX_MCP_SERVER': 'true'}, clear=True):
            with patch('cortex.core.environment_detector.EnvironmentDetector._is_mcp_server', return_value=True):
    ...
    def test_detect_copilot_environment(self, detector):
        ...
        with patch.dict(os.environ, {'TERM_PROGRAM': 'vscode'}, clear=True):
            with patch('cortex.core.environment_detector.EnvironmentDetector._is_mcp_server', return_value=False):
                with patch('cortex.core.environment_detector.EnvironmentDetector._is_copilot', return_value=True):
    ...
    def test_detect_development_environment(self, detector):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('cortex.core.environment_detector.EnvironmentDetector._is_mcp_server', return_value=False):
                with patch('cortex.core.environment_detector.EnvironmentDetector._is_copilot', return_value=False):
                    detector._cached_environment = None  # Clear cache
                    env_type = detector.detect_environment()

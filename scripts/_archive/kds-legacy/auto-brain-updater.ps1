...
if (-not $Silent) {
    Write-Host "`n🧠 Auto BRAIN Updater (Rule #22)" -ForegroundColor Cyan
}
...
if (-not $Silent) {
    Write-Host "✅ Request logged to events.jsonl" -ForegroundColor Green
}
...
if (-not $Silent) {
    Write-Host "📊 Events: $eventCount (threshold: 50)" -ForegroundColor Gray
    Write-Host "⏰ Hours since update: $([Math]::Round($hoursSinceUpdate, 1)) (threshold: 24)" -ForegroundColor Gray
}

# === STEP 3: Sync brain-updater Script (Keep Up-to-Date) ===
if (-not $Silent) {
    Write-Host "`n🔄 Checking brain-updater script sync..." -ForegroundColor Cyan
}
...
if ($needsScriptCreation) {
    if (-not $Silent) {
        Write-Host "⚠️  brain-updater.ps1 does not exist - needs creation" -ForegroundColor Yellow
...
Write-Host "`n🧠 BRAIN Updater v2.0 (Three-Tier)" -ForegroundColor Cyan
...
if (-not (Test-Path $eventsPath)) {
    Write-Host "❌ events.jsonl not found" -ForegroundColor Red
    exit 1
}
...
Write-Host "📊 New events to process: $($newEvents.Count)" -ForegroundColor Cyan

if ($newEvents.Count -eq 0 -and -not $Force) {
    Write-Host "✅ No new events - BRAIN up to date" -ForegroundColor Green
    exit 0
}

# Process events (simplified - full logic in brain-updater.md)
Write-Host "⚙️  Processing events..." -ForegroundColor Yellow
...
'@
    ...
    if (-not $Silent) {
        Write-Host "✅ Created brain-updater.ps1 wrapper" -ForegroundColor Green
    }
}
elseif ($needsScriptUpdate) {
    if (-not $Silent) {
        Write-Host "⚠️  brain-updater.md modified - updating wrapper script..." -ForegroundColor Yellow
    }
    ...
    if (-not $Silent) {
        Write-Host "✅ brain-updater.ps1 synchronized" -ForegroundColor Green
    }
}
else {
    if (-not $Silent) {
        Write-Host "✅ brain-updater.ps1 is up-to-date" -ForegroundColor Green
    }
}
...
if ($eventThresholdMet -or $timeThresholdMet) {
    if (-not $Silent) {
        Write-Host "`n⚡ Threshold met - Triggering brain-updater..." -ForegroundColor Yellow
    ...
    try {
        ...
        if (-not $Silent) {
            Write-Host "✅ BRAIN update completed" -ForegroundColor Green
        }
    }
    catch {
        if (-not $Silent) {
            Write-Host "⚠️  brain-updater.ps1 execution failed: $_" -ForegroundColor Yellow
    ...
    if (-not $Silent) {
        Write-Host "`n✅ No BRAIN update needed" -ForegroundColor Green

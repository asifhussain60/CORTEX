...
🚫 **INCREMENTAL_PLAN_GENERATION**: Incremental YAML Plan Generation
...
🚫 **BRAIN_PROTECTION_TESTS_MANDATORY**: Brain Protection Tests - 100% Pass Rate Mandatory
   - *Brain protection tests MUST pass - no exceptions*
   - Severity: `blocked`

⚠️ **MACHINE_READABLE_FORMATS**: Use Machine-Readable Formats for Efficiency
   - *Non-user files should use YAML/JSON, not Markdown*
   - Severity: `warning`

⚠️ **ACTIVE_NARRATOR_VOICE**: Active Narrator Voice (Not Passive Documentation)
   - *Story uses passive/clinical narrator voice instead of active storytelling*
   - Severity: `warning`

🚫 **CORTEX_PROMPT_FILE_PROTECTION**: CORTEX.prompt.md Protection (Never Rename, Safe Update)
...
🚫 **TIER0_APPLICATION_DATA**: No Application Data in Tier 0
   - *Application-specific path in Tier 0 (immutable governance)*
   - Severity: `blocked`

⚠️ **TIER2_CONVERSATION_DATA**: No Conversation Data in Tier 2
...
⚠️ **LEFT_BRAIN_TACTICAL**: Left Brain Tactical Only
   - *Strategic planning logic in tactical executor*
   - Severity: `warning`

⚠️ **RIGHT_BRAIN_STRATEGIC**: Right Brain Strategic Only
...
🚫 **SKULL_TEST_BEFORE_CLAIM**: Test Before Claim (SKULL-001)
   - *Never claim a fix is complete without test validation*
   - Severity: `blocked`

🚫 **SKULL_INTEGRATION_VERIFICATION**: Integration Verification (SKULL-002)
   - *Integration must be tested end-to-end*
   - Severity: `blocked`

⚠️ **SKULL_VISUAL_REGRESSION**: Visual Regression (SKULL-003)
   - *CSS/UI changes require visual validation*
   - Severity: `warning`

⚠️ **SKULL_RETRY_WITHOUT_LEARNING**: Retry Without Learning (SKULL-004)
   - *Must diagnose failures before retrying same approach*
   - Severity: `warning`

🚫 **SKULL_TRANSFORMATION_VERIFICATION**: Transformation Verification (SKULL-005)
   - *Operations claiming transformation MUST produce measurable changes*
   - Severity: `blocked`

🚫 **SKULL_PRIVACY_PROTECTION**: Privacy Protection (SKULL-006)
   - *Publish operations MUST NOT include files with machine-specific paths or private data*
   - Severity: `blocked`

🚫 **SKULL_HEADER_FOOTER_IN_RESPONSE**: Faculty Integrity Check (SKULL-007)
   - *Publish package MUST contain ALL essential CORTEX faculties for full operation*
   - Severity: `blocked`

🚫 **SKULL_HEADER_FOOTER_IN_RESPONSE_LEGACY**: Header/Footer in Copilot Response (Legacy)
   - *Operation orchestrators MUST include formatted headers/footers in Copilot Chat response*
   - Severity: `blocked`

🚫 **SKULL_ALL_TESTS_MUST_PASS**: All Tests Must Pass (SKULL-007)
   - *Test suite MUST have 100% pass rate before claiming any work complete*
   - Severity: `blocked`

🚫 **SKULL_MULTI_TRACK_VALIDATION**: Multi-Track Configuration Validation (SKULL-008)
   - *Multi-track mode MUST have valid configuration with proper phase distribution*
   - Severity: `blocked`

🚫 **SKULL_TRACK_ISOLATION**: Track Work Isolation (SKULL-009)
   - *Work on Track A MUST NOT modify Track B's assigned modules*
   - Severity: `blocked`

🚫 **SKULL_CONSOLIDATION_INTEGRITY**: Track Consolidation Integrity (SKULL-010)
...
⚠️ **MIN_OCCURRENCES**: Minimum Occurrences for High Confidence
   - *High confidence (>0.50) with single occurrence*
   - Severity: `warning`

⚠️ **PATTERN_VALIDATION**: Pattern Validation
...
⚠️ **BRAIN_STATE_GITIGNORE**: Brain State Files Not Committed
   - *Brain state file should not be committed*
   - Severity: `warning`

⚠️ **TEMP_FILES_COMMIT**: Temporary Files Not Committed
...
🚫 **GIT_ISOLATION_ENFORCEMENT**: CORTEX Code Isolation from User Repos
   - *CRITICAL: CORTEX source code, brain files, or internal components being committed to user application repository*
   - Severity: `blocked`

🚫 **GIT_HOOKS_INSTALLATION**: Git Hooks Must Be Installed During Setup
...
⚠️ **NAMESPACE-002**: Workspace Isolation
...
🚫 **DISTRIBUTED_DATABASE_ARCHITECTURE**: Use Tier-Specific Databases (Never Monolithic)
...
This section provides information about pragmatic mvp approach. See related documentation in the navigation menu for detailed guides.

### Test Optimization Patterns

This section provides information about test optimization patterns. See related documentation in the navigation menu for detailed guides.
...
**Evidence:** 18 failures → 0 failures in 6 hours using this approach
...
**Evidence:** 91.4% → 92.1% → 92.8% → 93.0% pass rate progression
...
This section provides information about architecture optimization patterns. See related documentation in the navigation menu for detailed guides.

...
<body>
    <div class="epic-container">
        ...
        <div class="epic-header">
            <h1 class="epic-title">≡ƒÄ» {config.plan_name}</h1>
    ...
    <div class="auto-refresh-indicator pulse">
        ≡ƒöä Auto-refreshing every {config.refresh_interval_ms // 1000}s
    </div>
...
<body>
    <div class="feature-container">
        ...
        <div class="feature-header">
            <h1 class="feature-title">≡ƒôï {config.plan_name}</h1>
            <div class="feature-meta">
                <span class="meta-item" id="feature-status">ΓÅ│ Not Started</span>
        ...
        <!-- Artifacts & Reports -->
        <div class="glass-panel">
            <h2>≡ƒôü Artifacts & Reports</h2>
            <div class="artifacts-grid">
                <a href="context/" class="artifact-link">≡ƒôû Context</a>
                <a href="artifacts/" class="artifact-link">≡ƒöº Artifacts</a>
                <a href="reports/" class="artifact-link">≡ƒôè Reports</a>
                <a href="tracking/" class="artifact-link">≡ƒôê Tracking</a>
            </div>
        </div>
    </div>
            ...
            childPlans.forEach(plan => {{
                ...
                card.innerHTML = `
                    <div class="child-plan-header">
                        ...
                        <span class="child-plan-status">${{plan.status_emoji || 'ΓÅ│'}}</span>
                    </div>
                    ...
                    <p class="child-plan-meta">
                        ${{plan.phases_complete || 0}}/${{plan.total_phases || 0}} phases ΓÇó ${{plan.duration || 'TBD'}}
                    </p>
    ...
    def _get_feature_javascript(self, config: ViewerConfig) -> str:
        ...
        function getStatusText(status) {{
            const statusMap = {{
                'not_started': 'ΓÅ│ Not Started',
                'in_progress': '≡ƒöä In Progress',
                'blocked': '≡ƒöÆ Blocked',
                'complete': 'Γ£à Complete',
                'failed': 'Γ¥î Failed'
            }};
            return statusMap[status] || 'ΓÅ│ Not Started';
        }}
...
if __name__ == "__main__":
    ...
    if viewer_type == "epic":
        ...
        print(f"Γ£à Generated epic viewer: {result}")
    elif viewer_type == "feature":
        result = generate_feature_viewer(plan_id, plan_name, output_path)
        print(f"Γ£à Generated feature viewer: {result}")
    else:
        print(f"Γ¥î Invalid type: {viewer_type} (must be 'epic' or 'feature')")
        sys.exit(1)

...
2. **App.Example.Domain** ⭐ **CORE DOMAIN**
   - Type: Domain Layer
   ...
   - Services: 11 domain services (ExampleDomainService, ExampleSettingsService, etc.)
...
- **Customer Context:** `App.Example.Domain` (customer-side operations, rollover logic)
...
**Domain Layer (App.Example.Domain):**
- Entities:
...
- Services:
  - `ExampleDomainService` (rollover business logic)
  - `ExampleSettingsService` (configuration management)
  - `ExampleSharedService` (shared utilities)

- Interfaces:
  - `IExampleDomainService`
  - `IExampleSettingsService`
  - `IExampleSharedService`
...
**Test Layer:**
- `App.Example.Domain.Tests` (rollover logic unit tests)
...
1. Rollover.Jobs (scheduled)
   ...
   → Calls ExampleDomainService.ProcessCarryover()
...
│   ├── → App.Example.Domain (ExampleDomainService)
│   └── → App.PaymentAccounts.Contracts
...
Libs (2)
├── App.Example.Domain (25 entities, 11 services)
...
**Component:** `App.Example.Domain` → `GlobalContributionMaxByYear` entity
**Validation Required:**
- Does `Rollover.Jobs` load GlobalContributionMaxByYear?
- Does `ExampleDomainService` enforce FlexAccount $3,200 / HealthSavings $4,150-$8,300 limits?
- Are limits validated BEFORE rollover calculations?

### P0: FlexAccount $640 Rollover Limit
**Component:** `App.Example.Domain` → `RolloverSettings` entity
**Validation Required:**
- Does `ExampleSettingsService` load $640 FlexAccount limit?
- Does `ExampleDomainService.ProcessCarryover()` validate rollover ≤ $640?
...
- **Analyze ExampleDomainService methods**

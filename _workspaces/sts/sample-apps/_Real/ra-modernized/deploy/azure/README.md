# RA Funding Invoices - Azure Resource Deployment Guide
...
This guide covers deploying Azure infrastructure for RA Funding Invoices modernization:
...
| `resourceGroupName` | Resource group name | `rg-ra-funding-invoices-{env}` |
...
**Name:** `ra-funding-invoices-config-{env}`  
...
**Name:** `ra-funding-invoices-insights-{env}`  
**Features:**
- Connected to Log Analytics workspace
- 90-day retention
- 100% sampling (no data loss)
- IP masking enabled (HIPAA compliance)
- Web application type

### 3. Azure Key Vault
**Name:** `ra-funding-invoices-kv-{env}`  
...
4. Select member: Your API app service managed identity
...
APP_IDENTITY=$(az webapp identity show \
    --name ra-funding-invoices-api-prod \
    --resource-group rg-ra-funding-invoices-prod \
    --query principalId \
    --output tsv)
...
az role assignment create \
    ...
    --scope /subscriptions/{SUBSCRIPTION-ID}/resourceGroups/rg-ra-funding-invoices-prod/providers/Microsoft.KeyVault/vaults/ra-funding-invoices-kv-prod
```
...
```json
{
  "AzureAppConfiguration": {
    "ConnectionString": "@Microsoft.KeyVault(SecretUri=https://ra-funding-invoices-kv-prod.vault.azure.net/secrets/AppConfiguration--ConnectionString/)"
  },
  "ApplicationInsights": {
    "ConnectionString": "@Microsoft.KeyVault(SecretUri=https://ra-funding-invoices-kv-prod.vault.azure.net/secrets/ApplicationInsights--ConnectionString/)"
  },
  "AzureKeyVault": {
    "Url": "https://ra-funding-invoices-kv-prod.vault.azure.net/"
  }
}
```
...
az appconfig feature show \
    --name ra-funding-invoices-config-prod \
...
az keyvault secret show \
    --vault-name ra-funding-invoices-kv-prod \
...
```bash
az group delete \
    --name rg-ra-funding-invoices-dev \
    --yes \
    --no-wait
```
...
az appconfig delete \
    --name ra-funding-invoices-config-dev \
    --resource-group rg-ra-funding-invoices-dev \
    --yes

# Delete Application Insights
az monitor app-insights component delete \
    --app ra-funding-invoices-insights-dev \
    --resource-group rg-ra-funding-invoices-dev

# Delete Key Vault (with purge protection)
az keyvault delete \
    --name ra-funding-invoices-kv-dev \
    --resource-group rg-ra-funding-invoices-dev
...
```bash
az group create --name rg-ra-funding-invoices-dev --location eastus2
```

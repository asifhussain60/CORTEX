...
notepad src/RA.FundingInvoices.API/RA.FundingInvoices.API.csproj
...
git checkout src/RA.FundingInvoices.API/RA.FundingInvoices.API.csproj
```
...
**Symptoms:**
```
No test is available in C:\...\RA.FundingInvoices.UnitTests.dll
```
...
dotnet clean tests/RA.FundingInvoices.UnitTests/
dotnet build tests/RA.FundingInvoices.UnitTests/

# Solution 2: Verify xUnit package
dotnet add tests/RA.FundingInvoices.UnitTests/ package xunit
dotnet add tests/RA.FundingInvoices.UnitTests/ package xunit.runner.visualstudio
...
**Symptoms:**
```
SqlException: Cannot open database "FundingInvoices_Test" requested by the login
```
...
dotnet ef migrations list --context FundingInvoicesDbContext

# If no migrations:
dotnet ef migrations add InitialCreate --context FundingInvoicesDbContext
dotnet ef database update --context FundingInvoicesDbContext
```
...
sqlcmd -S "(localdb)\MSSQLLocalDB" -d FundingInvoices_Test -Q "SELECT * FROM __EFMigrationsHistory"

# Solution 3: Remove last migration (if not applied to prod)
dotnet ef migrations remove --context FundingInvoicesDbContext
```
...
var invoice = await _context.FundingInvoices.FirstAsync(i => i.InvoiceId == id);

// ✅ Good: Safe with null check
var invoice = await _context.FundingInvoices.FirstOrDefaultAsync(i => i.InvoiceId == id);
if (invoice == null)
    throw new NotFoundException($"Invoice {id} not found");

// ✅ Good: Use SingleOrDefaultAsync for unique queries
var invoice = await _context.FundingInvoices.SingleOrDefaultAsync(i => i.InvoiceNumber == number);
```
...
# File: src/RA.FundingInvoices.API/Properties/launchSettings.json
{
...
Test-Path src/RA.FundingInvoices.API/appsettings.json
Test-Path src/RA.FundingInvoices.API/appsettings.Development.json
...
# Solution 4: Check launch.json (VS Code)
{
  ...
  "request": "launch",
  "preLaunchTask": "build",
  "program": "${workspaceFolder}/src/RA.FundingInvoices.API/bin/Debug/net8.0/RA.FundingInvoices.API.dll",
  "args": [],
  "cwd": "${workspaceFolder}/src/RA.FundingInvoices.API",
  "stopAtEntry": false,
  "serverReadyAction": {
...
var invoices = await _context.FundingInvoices.ToListAsync();
foreach (var invoice in invoices)
{
    var batch = await _context.FundingBatches.FindAsync(invoice.BatchId);  // N queries!
}
...
{
    private readonly DbContext _context = new FundingInvoicesDbContext();
}
...
services.AddSingleton<FundingInvoicesDbContext>();

// ✅ Good: Scoped DbContext
services.AddScoped<FundingInvoicesDbContext>();
...
NullReferenceException: Object reference not set to an instance of an object
  at FundingInvoiceService.CreateAsync(...)
```
...
### Issue: "No service for type 'IFundingInvoiceRepository'"

**Symptoms:**
```
InvalidOperationException: Unable to resolve service for type 
'RA.FundingInvoices.Core.Interfaces.IFundingInvoiceRepository' while attempting 
to activate 'RA.FundingInvoices.Core.Services.FundingInvoiceService'
```
...
public interface IFundingInvoiceRepository  // ← Interface name
{
    // ...
}

public class MockFundingInvoiceRepository : IFundingInvoiceRepository  // ← Implements interface
{
    // ...
}
...
services.AddScoped<IFundingInvoiceRepository, ...>();  // Scoped
services.AddScoped<IFundingInvoiceService, ...>();     // Scoped

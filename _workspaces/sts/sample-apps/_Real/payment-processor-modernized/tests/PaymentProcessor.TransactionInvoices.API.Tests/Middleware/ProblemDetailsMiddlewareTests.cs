...
using PaymentProcessor.TransactionInvoices.Core.Exceptions;
using System.Text.Json;
using Xunit;

namespace PaymentProcessor.TransactionInvoices.API.Tests.Middleware;
...
public class ProblemDetailsMiddlewareTests
{
    ...
    public async Task InvokeAsync_NotFoundException_Returns404WithProblemDetails()
    {
        ...
        var context = new DefaultHttpContext();
        context.Response.Body = new MemoryStream();
        context.Request.Path = "/api/v1/transaction-invoices/INV-999";

        var notFoundException = new NotFoundException("TransactionInvoice", "INV-999");
        ...
        problemDetails.GetProperty("instance").GetString().Should().Be("/api/v1/transaction-invoices/INV-999");
    }
    ...
    public async Task InvokeAsync_InvalidOperationException_Returns409WithProblemDetails()
    {
        ...
        var context = new DefaultHttpContext();
        context.Response.Body = new MemoryStream();
        context.Request.Path = "/api/v1/transaction-batches/close";

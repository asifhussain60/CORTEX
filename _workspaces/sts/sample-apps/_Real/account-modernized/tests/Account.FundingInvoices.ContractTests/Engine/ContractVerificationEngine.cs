...
using Account.FundingInvoices.ContractTests.Reporting;

namespace Account.FundingInvoices.ContractTests.Engine;
...
public class ContractVerificationEngine
{
    ...
    private async Task<WcfResponse> ExecuteWcfOperationAsync(string transaction, object request)
    {
        return transaction switch
        {
            ...
            "Updater_CreateAccountFundingInvoices" => await _wcfProxy.CreateBatchInvoicesAsync(request),
    ...
    private RestEndpoint GetRestEndpoint(string wcfTransaction)
    {
        return wcfTransaction switch
        {
            ...
            "Updater_CreateAccountFundingInvoices" => new RestEndpoint { Method = "POST", Path = "/api/v1/funding-invoices/batch" },

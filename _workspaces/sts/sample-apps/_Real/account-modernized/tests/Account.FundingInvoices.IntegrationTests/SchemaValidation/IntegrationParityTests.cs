...
using Account.FundingInvoices.Core.Entities;
using Account.FundingInvoices.Core.Interfaces;
using Account.FundingInvoices.Infrastructure.Mock;
using Account.FundingInvoices.Infrastructure.Persistence;
using Account.FundingInvoices.Infrastructure.Persistence.Repositories;
using Xunit;

namespace Account.FundingInvoices.IntegrationTests.SchemaValidation;
...
public class IntegrationParityTests : IDisposable
{
    ...
    public async Task GetInvoiceById_MockVsEFCore_MustReturnIdenticalData(string invoiceId)
    {
        ...
        mockResult.SubaccountId.Should().Be(efResult.SubaccountId);
    ...
    {
        // Arrange
        var mockRepo = new MockFundingBatchRepository();
        var efCoreRepo = new EFCoreFundingBatchRepository(_dbContext);
    ...
    public async Task GetSubaccountById_MockVsEFCore_MustReturnIdenticalData(string subaccountId)
    {
        // Arrange
        var mockRepo = new MockSubaccountRepository();
        var efCoreRepo = new EFCoreSubaccountRepository(_dbContext);

        // Act
        var mockResult = await mockRepo.GetByIdAsync(subaccountId);
        var efResult = await efCoreRepo.GetByIdAsync(subaccountId);
        ...
        mockResult.Should().NotBeNull($"Mock repository should return subaccount {subaccountId}");
        efResult.Should().NotBeNull($"EF Core repository should return subaccount {subaccountId}");

        mockResult!.SubaccountId.Should().Be(efResult!.SubaccountId);
        mockResult.SubaccountName.Should().Be(efResult.SubaccountName);
    ...
    public async Task CreateInvoice_MockVsEFCore_MustBehaveSimilarly()
    {
        ...
        var efInvoice = new FundingInvoice
        {
    ...
    private void SeedDatabaseWithMockData()
    {
        ...
        _dbContext.FundingBatches.AddRange(
            new FundingBatch { BatchId = "MOCK-BATCH-001", BatchName = "Test Batch 1", Status = "Active", CreatedDate = DateTime.UtcNow },
            new FundingBatch { BatchId = "MOCK-BATCH-002", BatchName = "Test Batch 2", Status = "Active", CreatedDate = DateTime.UtcNow },
            new FundingBatch { BatchId = "MOCK-BATCH-003", BatchName = "Test Batch 3", Status = "Active", CreatedDate = DateTime.UtcNow }
        );

        // Seed subaccounts
        _dbContext.Subaccounts.AddRange(
            new Subaccount { SubaccountId = "MOCK-SUB-001", SubaccountName = "Test Subaccount 1", Balance = 10000.00m, CreatedDate = DateTime.UtcNow },
            new Subaccount { SubaccountId = "MOCK-SUB-002", SubaccountName = "Test Subaccount 2", Balance = 20000.00m, CreatedDate = DateTime.UtcNow },
            new Subaccount { SubaccountId = "MOCK-SUB-003", SubaccountName = "Test Subaccount 3", Balance = 30000.00m, CreatedDate = DateTime.UtcNow }
        );

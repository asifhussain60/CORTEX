...
using Account.FundingInvoices.API;
...
public class UIContractTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task FundingInvoiceAPI_MockVsEFCore_MustReturnIdenticalJSONShape()
    {
        // Arrange
        var mockClient = _factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                var descriptor = services.SingleOrDefault(d => d.ServiceType == typeof(IFundingInvoiceRepository));
                if (descriptor != null)
        ...
        // Act
        var mockResponse = await mockClient.GetAsync("/api/v1/funding-invoices/MOCK-INVOICE-001");
        var efResponse = await efCoreClient.GetAsync("/api/v1/funding-invoices/MOCK-INVOICE-001");
    ...
    [Fact]
    public async Task FundingInvoiceListAPI_MockVsEFCore_MustReturnIdenticalArrayStructure()
    {
        // Arrange
        ...
        // Act
        var mockResponse = await mockClient.GetAsync("/api/v1/funding-invoices");
        var efResponse = await efCoreClient.GetAsync("/api/v1/funding-invoices");
    ...
    [Theory]
    [InlineData("/api/v1/funding-batches/MOCK-BATCH-001")]
    [InlineData("/api/v1/subaccounts/MOCK-SUB-001")]
    ...
    public async Task ErrorResponses_MockVsEFCore_MustHaveIdenticalFormat()
    {
        ...
        // Act
        var mockResponse = await mockClient.GetAsync("/api/v1/funding-invoices/DOES-NOT-EXIST");
        var efResponse = await efCoreClient.GetAsync("/api/v1/funding-invoices/DOES-NOT-EXIST");
    ...
    public async Task CreateInvoiceAPI_MockVsEFCore_MustAcceptIdenticalPayload()
    {
        ...
        // Act
        var mockResponse = await mockClient.PostAsJsonAsync("/api/v1/funding-invoices", payload);
        ...
        var efResponse = await efCoreClient.PostAsJsonAsync("/api/v1/funding-invoices", efPayload);
        ...
        return _factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                var descriptors = services.Where(d =>
                    d.ServiceType == typeof(IFundingInvoiceRepository) ||
                    d.ServiceType == typeof(IFundingBatchRepository) ||
                    d.ServiceType == typeof(ISubaccountRepository)).ToList();

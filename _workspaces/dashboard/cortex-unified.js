...
// ========== Initialization ==========
function initialize() {
    console.log('≡ƒÜÇ Initializing CORTEX Dashboard');
    initializeData();
    initializeTabs();
    initializeTheme();
    initializeCharts();
    initializeNarratives();
    console.log('≡ƒºá CORTEX Dashboard initialized');
}
...
function initializeData() {
    ...
    if (dataScript) {
        try {
            ...
            console.log('≡ƒôè Analysis data loaded:', Object.keys(analysisData));
        } catch (error) {
            console.error('Failed to load analysis data:', error);
        }
    } else {
        console.log('ΓÜá∩╕Å No analysisData script found');
    }
}

// ========== Tab Navigation ==========
function initializeTabs() {
    console.log('≡ƒöº Initializing tabs...');
    ...
    if (tabButtons.length === 0) {
        console.error('Γ¥î No tab buttons found! Check HTML structure.');
        return;
    }
    ...
    tabButtons.forEach((button, index) => {
        ...
        button.addEventListener('click', () => {
            ...
            if (targetContent) {
                ...
                console.log(`Γ£à Activated tab: ${targetTab}`);
            } else {
                console.error(`Γ¥î Tab content not found: ${targetTab}`);
            }
    ...
    if (initialButton) {
        initialButton.click();
        console.log(`Γ£à Clicked initial tab: ${initialTab}`);
    } else {
        console.error(`Γ¥î Initial tab button not found: ${initialTab}`);
    }
}
...
function updateThemeIcon(theme) {
    ...
    if (themeIcon) {
        themeIcon.textContent = theme === 'dark' ? '≡ƒîÖ' : 'ΓÿÇ∩╕Å';
    }
}
...
function exportToPDF() {
    console.log('≡ƒôä Exporting to PDF...');
    window.print();
}

function exportToJSON() {
    console.log('≡ƒÆ╛ Exporting to JSON...');
...
function initializeNarratives() {
    ...
    console.log('≡ƒôè Business narratives rendered');
}
    ...
    container.innerHTML = useCases.map(useCase => `
        <div class="use-case-card glass-card">
            ...
            <div class="use-case-meta">
                <span class="use-case-actor">≡ƒæñ ${escapeHtml(useCase.actor)}</span>
                <span class="use-case-trigger">≡ƒöö ${escapeHtml(useCase.trigger)}</span>
            </div>
...
function renderCompetitiveAdvantages(competitive) {
    ...
    container.innerHTML = competitive.advantages.map(adv => `
        <div class="competitive-item">
            <div class="competitive-icon">Γ£¿</div>

...
"Create a ~6 minute hands-on tutorial for first-time users: prerequisites, clone/setup, MCP configuration, first run of /audit fix, then a quick smoke test. Keep narration warm and instructional; narration must explain why each step matters and common gotchas rather than reading commands. Use only provided sources; any terminal output should be realistic and plausible." 
...
NotebookLM produces **AI-narrated slides**. Treat the visuals as clean, readable tutorial cards.

- Prefer simple motion (gentle pans/zooms) over deterministic “camera choreography”.
- If you want a true VS Code demo feel, plan to **screen record** the real steps and stitch it in later.

## CORTEX voice (tutorial)
- Narrator should sound like a **senior project lead**: calm, patient, and practical.
- Avoid sales language. Use plain verbs and explain *why the step matters*.

## SDLC templates visual (when they appear)
- If you reference workflow templates or gates, show them as **YAML/JSON config** cards.

## Define the closure rule (Audit → Fix → Rescan)
- **Audit**: show violations.
- **Fix**: resolve them (possibly iteratively).

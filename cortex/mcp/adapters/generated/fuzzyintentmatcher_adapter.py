"""
MCP Adapter for FuzzyIntentMatcher

Generated adapter for FuzzyIntentMatcher.
AC-ID: AC-PHASE2B-005
"""

from typing import Any, Dict, List, Optional
from cortex.mcp.orchestrator_mcp_server import (
    IOrchestratorAdapter,
    CapabilityMetadata,
    CapabilityResponse,
    ExecutionContext,
)
from cortex.orchestrators.core.fuzzy_intent_matcher import FuzzyIntentMatcher
import logging
import time

logger = logging.getLogger(__name__)


def _get_orchestrator_from_wiring(name: str) -> Optional[Any]:
    """Get orchestrator from wiring system (CORE-035: Single execution path)."""
    try:
        from cortex.wiring import bootstrap_cortex
        registry = bootstrap_cortex()
        return registry.get_orchestrator(name)
    except Exception as e:
        logger.warning(f"Failed to get {name} from wiring: {e}")
        return None


class FuzzyIntentMatcherAdapter(IOrchestratorAdapter):
    """
    MCP Adapter for FuzzyIntentMatcher.
    
    Exposes capabilities:
    - match_intent: Fuzzy match user intent
    - get_suggestions: Get intent suggestions
    
    CORE-035: Uses wiring system for orchestrator access (single execution path).
    """
    
    def __init__(self, orchestrator: Optional[FuzzyIntentMatcher] = None):
        """Initialize adapter with orchestrator from wiring system."""
        if orchestrator is not None:
            self.orchestrator = orchestrator
        else:
            self.orchestrator = _get_orchestrator_from_wiring("FuzzyIntentMatcher")
        self.name = "FuzzyIntentMatcherAdapter"
    
    def get_capabilities(self) -> List[CapabilityMetadata]:
        """Get all capabilities exposed by this orchestrator."""
        return [
            CapabilityMetadata(
                name="match_intent",
                orchestrator="match",
                description="Fuzzy match user intent",
                input_schema={'query': {'type': 'string', 'description': 'query parameter'}, 'threshold': {'type': 'string', 'description': 'threshold parameter'}},
                output_schema={"status": {"type": "string"}, "result": {"type": "object"}},
                routing_keywords=["match_intent", "match intent"],
                tags={"generated", "phase2b"},
            ),
            CapabilityMetadata(
                name="get_suggestions",
                orchestrator="get",
                description="Get intent suggestions",
                input_schema={'partial_query': {'type': 'string', 'description': 'partial_query parameter'}},
                output_schema={"status": {"type": "string"}, "result": {"type": "object"}},
                routing_keywords=["get_suggestions", "get suggestions"],
                tags={"generated", "phase2b"},
            )
        ]
    
    def execute_capability(
        self,
        capability_name: str,
        parameters: Dict[str, Any],
        context: ExecutionContext,
    ) -> CapabilityResponse:
        """Execute a capability."""
        start = time.time()
        try:
            if not self.orchestrator:
                return CapabilityResponse(
                    request_id=context.session_id,
                    success=False,
                    error="Orchestrator not available",
                    orchestrator="fuzzyintentmatcher",
                    duration_ms=(time.time() - start) * 1000,
                )
            
            if capability_name == "match_intent":
                result = self.orchestrator.match_intent(query=parameters.get('query'), threshold=parameters.get('threshold'))
                return CapabilityResponse(
                    request_id=context.session_id,
                    success=True,
                    result=result,
                    orchestrator="{orchestrator_name_lower}",
                    duration_ms=(time.time() - start) * 1000,
                )
            elif capability_name == "get_suggestions":
                result = self.orchestrator.get_suggestions(partial_query=parameters.get('partial_query'))
                return CapabilityResponse(
                    request_id=context.session_id,
                    success=True,
                    result=result,
                    orchestrator="{orchestrator_name_lower}",
                    duration_ms=(time.time() - start) * 1000,
                )
            
            return CapabilityResponse(
                request_id=context.session_id,
                success=False,
                error=f"Unknown capability: {capability_name}",
                orchestrator="fuzzyintentmatcher",
                duration_ms=(time.time() - start) * 1000,
            )
        except Exception as e:
            logger.error(f"Error executing {capability_name}: {e}")
            return CapabilityResponse(
                request_id=context.session_id,
                success=False,
                error=str(e),
                orchestrator="fuzzyintentmatcher",
                duration_ms=(time.time() - start) * 1000,
            )
    
    def validate_parameters(
        self, capability_name: str, parameters: Dict[str, Any]
    ) -> tuple[bool, Optional[str]]:
        """Validate parameters for a capability."""
        # Basic validation - can be enhanced per capability
        return True, None

...
from cortex.mcp.v2.registry import (
    ToolRegistry,
    PRODUCTION_TOOLS,
    ToolMetadata,
    get_registry,
)
from cortex.mcp.v2.server import MCPServerV2, MCPRequest, MCPResponse
...
def server(registry):
    ...
    return MCPServerV2(registry=registry)
...
class TestJSONRPCProtocol:
    ...
    def test_unknown_method_returns_error(self, server):
        ...
        assert response.error["code"] == MCPServerV2.METHOD_NOT_FOUND
    ...
    def test_handle_invalid_json_returns_parse_error(self, server):
        ...
        assert response["error"]["code"] == MCPServerV2.PARSE_ERROR
...
class TestCrossPlatform:
    ...
    def test_server_works_on_platform(self, platform, registry):
        ...
        with patch.object(sys, "platform", platform):
            server = MCPServerV2(registry=registry)

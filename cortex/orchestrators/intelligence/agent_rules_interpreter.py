...
Execution Flow:
  ┌─────────────────────────────────────────────────────────────┐
  │  1. Agent receives user request in CORTEX context           │
  ├─────────────────────────────────────────────────────────────┤
  │  2. AgentRulesInterpreter loads rules from registry         │
  │     (ExecutionContext: CORTEX_INTERNAL or PRODUCTION_REPO)  │
  ├─────────────────────────────────────────────────────────────┤
  │  3. Rules validation: enforcement level + constraints       │
  │     • BLOCKED: halt immediately on violation               │
  │     • PRE_EXECUTION: validate before orchestration         │
  │     • WARNING: log but continue                            │
  │     • RUNTIME: monitor during execution                    │
  ├─────────────────────────────────────────────────────────────┤
  │  4. ExecutionDirective compiled (action → orchestrator)     │
  ├─────────────────────────────────────────────────────────────┤
  │  5. Master Orchestrator delegates to target orchestrator    │
  │     (TDD, Refactoring, LENS, etc.)                         │
  ├─────────────────────────────────────────────────────────────┤
  │  6. Violation logged in audit trail (AC markers)            │
  └─────────────────────────────────────────────────────────────┘

Key Concepts:
  • ExecutionContext: CORTEX vs production repo rules differ
  • RuleEnforcementLevel: Controls violation handling (block/warn/log)
  • AgentRole: Architect, Auditor, Designer, Executor, Validator, Digest
  • RulesRegistry: YAML-based SSOT for all rules

Example Usage:
  interpreter = AgentRulesInterpreter(registry_path)
  directives = interpreter.interpret(request, context=ExecutionContext.CORTEX_INTERNAL)
...
class ExecutionContext(str, Enum):
    ...
    Usage:
      ...
      if context == ExecutionContext.CORTEX_INTERNAL:
          # Load stricter rules from cortex-internal registry
    """
...
class RuleEnforcementLevel(str, Enum):
    ...
    Decision Logic:
      ...
      elif violation.severity == PRE_EXECUTION:
          collect_violation()  # Continue validation phase
      elif violation.severity == RUNTIME:
          warn()  # Log and continue
      elif violation.severity == WARNING:
          log()   # Silent logging
      elif violation.severity == PRINCIPLE:
          pass    # Aspirational only
    """

...
class CleanupAuditor:
    ...
    def __init__(self, workspace_root: Path):
        ...
        self.cortex_brain_dir = workspace_root / "cortex_brain"
    ...
    def audit_workspace(self) -> AuditResult:
        ...
        # Step 4: Scan cortex_brain/ directory (if exists)
        if self.cortex_brain_dir.exists():
            click.echo("Scanning cortex_brain/ directory...")
            
            for py_file in self.cortex_brain_dir.rglob("*.py"):
                if "__pycache__" in str(py_file) or "__init__.py" == py_file.name:
                    continue

...
class ContentPopulationStrategy:
    ...
    def __init__(self, template_base_path: Optional[Path] = None):
        ...
        Args:
            template_base_path: Base path for template files (defaults to cortex_intelligence/tier2)
        """
        if template_base_path is None:
            self.template_base_path = Path(__file__).parent.parent.parent / "cortex_intelligence" / "tier2"
        else:

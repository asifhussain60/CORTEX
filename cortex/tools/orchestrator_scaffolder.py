    ...
    def _render_tests_via_intelligence(
        ...
        try:
            ...
            spec = OrchestratorSpec(
                ...
                stages=[s.get('name', f'stage_{i}') for i, s in enumerate(self._get_stages(template))],
                hooks=list(self._get_hooks(template).keys()),
            ...
            module_name = self._to_module_name(template.name)
            test_code = []
            test_code.append(f'"""Tests for {class_name}')
            test_code.append(f'Domain: {config.domain}')
            test_code.append('')
            test_code.append('Generated by CORTEX ScaffolderIntelligenceAdapter (WAVE-2)')
            test_code.append('"""')
            test_code.append('')
            test_code.append('import pytest')
            test_code.append('from pathlib import Path')
            test_code.append('import yaml')
            test_code.append('from unittest.mock import Mock, patch')
            test_code.append('')
            test_code.append(f'from cortex.orchestrators.{config.domain}.{module_name} import {class_name}')
            test_code.append('')
            test_code.append('')
            test_code.append(f'class Test{class_name}:')
            test_code.append(f'    """Test suite for {class_name}."""')
            test_code.append('')
            test_code.append('    @pytest.fixture(autouse=True)')
            test_code.append('    def setup(self):')
            test_code.append('        """Setup test fixtures."""')
            test_code.append(f'        self.orchestrator_class = {class_name}')
